...
<?php 
	...
	$query   = "SELECT
				...
				INNER JOIN poderes_asunto ON poderes_asunto.id_asunto = 

ingreso_poderes.id_asunto
				LEFT OUTER JOIN poderes_contratantes ON 

poderes_contratantes.id_poder=ingreso_poderes.id_poder
				LEFT OUTER JOIN c_condiciones ON c_condiciones.id_condicion = 

poderes_contratantes.c_condicontrat";
		
			if($nro_control <> "" and $num_crono <> ""){

				$query = $query." where ingreso_poderes.id_poder like '%

$nro_control%' or ingreso_poderes.num_kardex like '%$num_crono%'";
...
%'";
			...
			if($nro_control == "" and $num_crono <> ""){
				...
				$query = $query." where ingreso_poderes.num_kardex like '%
...
poderes_contratantes.c_descontrat like '%$contratante%'";
				...
				if($opcion==1 and $rango1<> "" and $rango2<> ""){
						if($contratante == ''){
							$query = $query." where STR_TO_DATE

(ingreso_poderes.fec_ingreso,'%Y-%m-%d')  >= STR_TO_DATE('$rango1','%Y-%m-%d') 
											  and 

STR_TO_DATE(ingreso_poderes.fec_ingreso,'%Y-%m-%d') <= STR_TO_DATE('$rango2','%Y-%m-%d')";
...
STR_TO_DATE(ingreso_poderes.fec_ingreso,'%Y-%m-%d') >= STR_TO_DATE('$rango1','%Y-%m-%d') 
											  and 
...
(ingreso_poderes.fec_crono,'%Y-%m-%d') >= STR_TO_DATE('$rango1','%Y-%m-%d') 
											  and 

STR_TO_DATE(ingreso_poderes.fec_crono,'%Y-%m-%d') <= STR_TO_DATE('$rango2','%Y-%m-%d')";
						}
						if($contratante <> ''){
							$query = $query." where 
...
STR_TO_DATE(ingreso_poderes.fec_crono,'%Y-%m-%d') >= STR_TO_DATE('$rango1','%Y-%m-%d') 
											  and 
...
STR_TO_DATE(ingreso_poderes.fec_crono,'%Y-%m-%d') <= STR_TO_DATE('$rango2','%Y-%m-%d')";
	  ...
	  $query = $query." GROUP BY ingreso_poderes.id_poder ORDER BY ingreso_poderes.id_poder 
...
height='auto'>
        ...
        <tr height='20'>
            <td height='20' width=10 align='center' ><span class='reskar' title='Ver' 

style='color:#06C; cursor:pointer' onclick="ver_poderes('<?php echo $arr_poderes[$j][0]?>')"><?php 

echo $arr_poderes[$j][0]; ?></span></td>
            <td height='20' align='center'><span class='reskar'><?php echo formato_crono_agui

($arr_poderes[$j][5]);?></span></td>
            <td height='20' align='center'><span class='reskar'><?php echo $arr_poderes[$j][6]; ?></span></td>
            <td height='20' align='center'><span class='reskar'><?php echo fechabd_an($arr_poderes
...
c_condiciones ON c_condiciones.id_condicion = poderes_contratantes.c_condicontrat
										WHERE
...
echo $contratantes["condicion"]." : ".$contratantes["contratante"];
  echo "</br>";}
        ...
        <tr height='25'>
            <td colspan='7' align='center' valign='bottom'>
                <table style='margin-bottom:4px'>
                   <tr class='paginacion'>
                    <?php if($pag>7){?>
                        <td><div class='pagina' style='cursor:pointer; width:14px' title='Regresar' 

onclick="buscar_poderes('<?php echo ($ini_pag-1); ?>')"><--</div></td>
                    <?php } 
                    for($i=$ini_pag; $i<$ini_pag+7; $i++){
                        ...
                        <td width='15'>
                            <?php	
                            if($i==$pag){ ?>
                            <div class='pagina' style='cursor:pointer; 14px; background-

color:#525252; color:'white' ' title='Ir a' onclick="buscar_poderes('<?php echo $i; ?>)"><u><?php 

echo $i; ?></u></div>
                            <?php	}else{ ?>
                            <div class='pagina' style='cursor:pointer' title='Ir a' 

onclick="buscar_poderes('<?php echo $i; ?>')"><?php echo $i; ?></div>
                            <?php } ?>
                        </td>
                        <?php }
                    }
                    if($num_pag>7 and ($ini_pag+7)<=$num_pag){	?>
                    <td><div class='pagina' style='cursor:pointer; width:14px' title='Continuar' 

onclick="buscar_poderes('<?php echo ($ini_pag+7); ?>')">--></div></td>
                    <?php
                    } 
                    ?>	  
                    </tr>
