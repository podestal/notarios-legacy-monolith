...
$consulta = mysql_query("SELECT m_cteventas.id_ctaventas AS 'idctaventas', DATE_FORMAT(m_cteventas.fecha,'%d/%m/%Y') AS 'Fec.Emision', (CASE WHEN ISNULL(m_regpagos.fec_pago) THEN 'NO PAGADO' ELSE DATE_FORMAT(m_regpagos.fec_pago,'%d/%m/%Y') END) AS 'Fec.Pago', tip_documen.des_docum AS 'Tipo', m_cteventas.serie AS 'Serie',
...
echo"<table width='834' border='1' cellpadding='0' cellspacing='0' >
  <tr>
	...
	<td width='10' align='right' valign='top'><a id=".$row['idctaventas']." name=".$row['Serie']."-".$row['N.Documen.']." href='#' onclick='Imprime(this.id, this.name);'  ><img src='../../iconos/impresora.png' alt='' width='15' height='15' border='0' /></a></td>
  </tr>
</table>";
}
