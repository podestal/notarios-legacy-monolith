// JavaScript Document
	...
	function listar_reporte_e(pag){
		...
		if(flag==1){
			...
			new Ajax.Updater("list_reporte_e", "../consultas/list_comprobantes_e.php?fechade="+fechade+"&fechaa="+fechaa+"&pag="+pag+"&filtro="+filtro);	
	...
	function pdf_reporte(){
		...
		if(flag==1){
			...
			window.open('../pdf/reporte_e1.php?'+campos);
		}
		...
		if(flag==1){
			var campos =$("frm_fechas").serialize();
			window.open('../pdf/reporte_e.php?'+campos);
		}
