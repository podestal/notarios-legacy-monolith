	...
	/*function actualizar_totales(){
		...
		for(var i=1; i<=indice; i++){
			if(total[i] != null){subtotal = subtotal + Math.round(total[i]*100)/100;}
		}
		...
		number = subtotal*0.18
		
		if($("tip_comp").value == 04 || $("tip_comp").value == 01){$("igv").value=0.00;		
		}else{$("igv").value = Math.round(number*100)/100;}
		
		number = subtotal*1.18
		
		if($("tip_comp").value == 04 || $("tip_comp").value == 01){$("total").value=subtotal;		
		}else{$("total").value =  Math.round(number*100)/100;}
...
function actualizar_totales(){
		...
		number = subtotal1*0.18
		
		$("subtotal").value = subtotal1-number;
