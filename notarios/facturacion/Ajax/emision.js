	...
	function cargar_contenido(){
		...
		new Ajax.Updater("slc_tipdoc", "../widgets/tipodocumento.php");	
		new Ajax.Updater("div_datoscliente", "../widgets/datoscliente.php");	
		
		new Ajax.Updater("div_detalleslc", "../widgets/detalleslc.php",
		{  
		  onComplete: 
		  function() { 
				new Ajax.Updater("slc_tippag", "../widgets/tipopago.php");	
				new Ajax.Updater("slc_area", "../widgets/area.php");	
				new Ajax.Updater("slc_usuario", "../widgets/usuarios.php");					
		  }
		  },{method:'GET'}
		);	
	...
	function cambiar_doic(id, valor){
		...
		jQuery('#txtCodeCaptcha').val('');
		jQuery('#nom_cliente').val('');
		jQuery('#direccion').val('');

		jQuery('#txtCodeCaptcha').hide();
		jQuery('#imgCaptcha').hide();
		jQuery('#btnSearchReniecSunat').hide();
		...
		switch (valor){
			...
			case '1':
				//jQuery('#imgCaptcha').attr('src','../../reniec_sunat/generate_captcha_reniec.php');
				//jQuery('#txtCodeCaptcha').show();
				//jQuery('#imgCaptcha').show();
				//jQuery('#btnSearchReniecSunat').show();
			...
			case '8':
				//jQuery('#imgCaptcha').attr('src','../../reniec_sunat/generate_captcha_sunat.php');
				//jQuery('#txtCodeCaptcha').show();
				//jQuery('#imgCaptcha').show();
				//jQuery('#btnSearchReniecSunat').show();
	...
	var sdoic = $("select_doic").value;
		...
		if (unicode == 13){
			...
			new Ajax.Updater("div_datoscliente", "../widgets/datoscliente.php?doic="+doic+"&sdoic="+sdoic,
			  {  
			  onComplete: 
	...
	function nuevo_cliente(){
		...
		if(sdoic!=8){
		   new Ajax.Updater("div_ncliente", "../consultas/nuevocliente.php?sdoic="+valor1+"&doic="+doic,
		       { 
			   onComplete: 
			   function() { 	   
			...
			new Ajax.Updater("div_ncliente", "../consultas/nuevocliente.php?sdoic="+valor2+"&doic="+doic,
		       { 
			   onComplete: 
			   function() { 	   
	...
	function cerrar_ncliente(){
		...
		new Ajax.Updater("div_ncliente", "../consultas/nuevocliente.php");	
	}
	...
	function grabar_cliente(){
		...
		new Ajax.Request("../consultas/registrar_cliente.php?"+campos+"&tip_per="+tip_per,
		{  
		  onComplete: 
		  function() { 
		  	enviar_clidatos();
	...
	function cambiar_tipopago(){
		...
		if(id_tipago=='5'){
			...
			new Ajax.Updater("div_checkpago", "../widgets/tipdocserienum.php");
		
		}else if(id_tipago=='4' || id_tipago=='7' || id_tipago=='8'){
		
			new Ajax.Updater("div_checkpago", "../widgets/bancocheque.php");	
...
function actualizar_totales(){
	...
	function grabar_comprobante(){
		...
		if(val==1){
			...
			new Ajax.Request('../consultas/grabar.php?'+campos+"&valorusu="+validador, 
			{
				method:'get',
	...
	function validar_comprobante(){
		...
		if(val != 2){
			if($("numdoc1").value=="" || $("numdoc2").value==""){
				alert("Numero vacío");
				val = 2;
			}
		}
   ...
   function cambiarazon(){
	...
	gCount = 1;
	jQuery('#btnSearchReniecSunat').live('click',function(e){
		
		vNumberDocument = jQuery('#doic').val();
		vCodeCaptcha = jQuery('#txtCodeCaptcha').val();
		vTypeDocument = jQuery('#select_doic').val();

		jQuery('#msg-error').hide();


		jQuery.ajax({
			url:'../../reniec_sunat/get_data_persona.php',
			type:'POST',
			data:{document:vNumberDocument,codeCaptcha:vCodeCaptcha,typeDocument:vTypeDocument},
			dataType:'json',
			success:function(response){
				...
				if(response.error == 0){
					...
					if(response.documentType == 1){
						jQuery('#nom_cliente').val(response.fullNames);
					}else{
						jQuery('#nom_cliente').val(response.businessName);
						jQuery('#direccion').val(response.address);
						jQuery('#txtCodeCaptcha').val('');
						jQuery('#imgCaptcha').attr('src','../../reniec_sunat/generate_captcha_sunat.php?gCount='+gCount);
						gCount++;
