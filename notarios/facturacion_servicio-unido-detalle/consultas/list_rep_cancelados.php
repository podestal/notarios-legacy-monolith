    ...
    while($cuentas = mysql_fetch_array($res_cuentas)){
		...
		$arr_cuentas[$i][9] = $cuentas["importe"]; 
		$arr_cuentas[$i][10] = $cuentas["movi"]; 
		$arr_cuentas[$i][11] = $cuentas["tip_pago"]; 
		$i++; 
    }
...
-->
		...
		<table width="870" border="1" cellspacing="0" cellpadding="0" >
              ...
              <td width="80" align="center"><span class="titubuskar0">Fec. Emisión</span></td>
          	  <td width="95" align="center"><span class="titubuskar0">Tipo Doc.</span></td>
              ...
              <td width="70" align="center"><span class="titubuskar0">Importe</span></td>
              <td width="60" align="center"><span class="titubuskar0">Tipo Mov.</span></td>
              <td width="95" align="center"><span class="titubuskar0">Tipo Pago</span></td>
              <td width="70" align="center"><span class="titubuskar0">Extornar</span></td>
          </tr>
          <?php
		  ...
		  for($j=0; $j<count($arr_cuentas); $j++){ ?>
         	 <tr id="fila_cancelar<?php echo $arr_cuentas[$j][0]; ?>">
                ...
                <td align='center'>
                	<span class='reskar'><?php echo $arr_cuentas[$j][10]; ?></span>
                </td>
                <td align='center'>
                	<span class='reskar'><?php echo $arr_cuentas[$j][11]; ?></span>
                </td>
                <td align='center'>
                	<span class='reskar'><img src="../../images/extornar.png" width="15" height="15" title="Extornar" style="cursor:pointer" onclick="login('<?php echo $arr_cuentas[$j][8]; ?>','<?php echo $arr_cuentas[$j][3]; ?>','<?php echo $arr_cuentas[$j][4]; ?>','<?php echo $arr_cuentas[$j][0]; ?>')"/></span>
                </td>
			</tr>
          ...
          <tr height='25'>
				    <td colspan='9' align='center' valign='bottom'>
					    <table style='margin-bottom:4px'>
						   <tr class='paginacion'>
						    <?php if($pag>7){?>
								<td><div class='pagina' style='cursor:pointer; width:14px' title='Regresar' onclick="listar_rep_cancelados('<?php echo ($ini_pag-1); ?>')"><--</div></td>
						    <?php } 
							for($i=$ini_pag; $i<$ini_pag+7; $i++){
								...
								<td width='15'>
									<?php	
									if($i==$pag){ ?>
									<div class='pagina' style='cursor:pointer; 14px; background-color:#525252; color:'white' ' title='Ir a' onclick="listar_rep_cancelados('<?php echo $i; ?>')"><u><?php echo $i; ?></u></div>
									<?php	}else{ ?>
									<div class='pagina' style='cursor:pointer' title='Ir a' onclick="listar_rep_cancelados('<?php echo $i; ?>')"><?php echo $i; ?></div>
                                	<?php } ?>
                                </td>
								<?php }
						    }
						  	if($num_pag>7 and ($ini_pag+7)<=$num_pag){	?>
							<td><div class='pagina' style='cursor:pointer; width:14px' title='Continuar' onclick="listar_rep_cancelados('<?php echo ($ini_pag+7); ?>')">--></div></td>
    						<?php
						    }
                            ?>	  
