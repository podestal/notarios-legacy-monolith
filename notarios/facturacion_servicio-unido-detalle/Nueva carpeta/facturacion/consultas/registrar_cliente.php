<?php
	...
	$sql_buscardoc = "SELECT cliente.idcliente, cliente.numdoc AS idcliente FROM cliente where cliente.numdoc=$numero_doc"; 
	...
	if($existe_doc==0){
		...
		if($per_natural<>""){
		   echo $sql_insertclientes = "INSERT 
								  INTO cliente
								(idcliente, prinom, segnom, apepat, apemat, nombre, numdoc, telfijo, telcel, direccion, email, sexo, idestcivil,  nacionalidad, cumpclie, idubigeo, residente, natper, tipper, idtipdoc,docpaisemi,tipocli)
								VALUES 
		...
		if($per_juridica <>""){
		   $sql_insertclientes = "INSERT 
								  INTO cliente
								  (idcliente, razonsocial, domfiscal, mailempresa, telofi, telcel, numdoc, fechaconstitu, idubigeo, tipper, idestcivil, tipocli)
								  VALUES 
