  ...
  $query  =    "SELECT
	...
	if($num_crono == ""){
		...
		if($cliente<> '' and $rn == ''){
			  if($rango1== "" or $rango2== ""){ 
				$query = $query." and libros.empresa like '%$cliente%' or CONCAT(libros.prinom, ' ',libros.segnom, ' ',libros.apepat, ' ',libros.apemat) like '%$cliente%'";  
			  }
			  if($rango1<> "" and $rango2<> ""){  
				$query = $query." and (libros.empresa like '%$cliente%' or CONCAT(libros.prinom, ' ',libros.segnom, ' ',libros.apepat, ' ',libros.apemat) like '%$cliente%') 
	...
	$query = $query." GROUP BY libros.numlibro ORDER BY libros.numlibro DESC "; 
    ...
    $arr_libros[$i][4] = $libros["nombre1"]." ".$libros["nombre2"];
	...
	$arr_libros[$i][11] = $libros["dni"];
	$arr_libros[$i][12] = $libros["ruc"];
	$arr_libros[$i][13] = $libros["descripcion"];
	$arr_libros[$i][14] = strtoupper($arr_libros[$i][4].' '.$arr_libros[$i][5].' '.$arr_libros[$i][6]);
    $i++; 
    }
	?>
    ...
    <table width='840' border='1' cellpadding='0' cellspacing='0' bordercolor='#333333' height='auto'>
     ...
     <tr height='20'>
            <td width='79' align='center'>
				<span class='reskar' title='Ver' style='color:#06C; cursor:pointer' onclick="ver_libros('<?php echo $arr_libros[$j][0]?>')"><?php echo $arr_libros[$j][0]."-".$arr_libros[$j][1];?></span>
            </td>
            ...
            <td width='153' align='center'>
                <?php
            if($arr_libros[$j][3]<>""){	?>		
                <span class='reskar'><?php echo $arr_libros[$j][3];?></span>
             <?php    
            }else{ ?>
                <span class='reskar'><?php echo $arr_libros[$j][14];?></span>
            <?php }
			?>
            </td>
			<td width='116' align='center'>
				<span class='reskar'><?php echo strtoupper($arr_libros[$j][13]); ?></span>
			</td>
			...
			<?php 			
            if(trim($arr_libros[$j][12])<>""){		
            ?>		
					<span class='reskar'><?php echo $arr_libros[$j][12]; ?></span>
			<?php              
            }else{
            ?>		
					<span class='reskar'><?php echo $arr_libros[$j][11]; ?></span>
			<?php               
            }?>
            </td>
            <td align="center">
