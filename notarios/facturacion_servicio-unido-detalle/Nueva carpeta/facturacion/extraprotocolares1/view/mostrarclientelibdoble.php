...
include("../../conexion.php");
	...
	$idclie=$_POST['idclie']; 
	$sqlconyuge  = mysql_query("SELECT cliente.nombre FROM cliente WHERE cliente.idcliente = '".$idclie."'", $conn) or die(mysql_error());
	$row_conyuge = mysql_fetch_array($sqlconyuge);	
	$conyuge_imprime = $row_conyuge[0];
	
	$sqlclie=mysql_query("select * from cliente where idcliente = '".$idclie."'", $conn) or die(mysql_error());
