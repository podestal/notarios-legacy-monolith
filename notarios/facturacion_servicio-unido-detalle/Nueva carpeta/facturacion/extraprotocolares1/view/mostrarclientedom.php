...
<!-- aca empieza la webada-->
	<table>
		...
		?>
          ...
          <td colspan="3"><input name="distrito_solic" value="<?php echo $res['coddis'];?>" type="hidden" id="distrito_solic" size="15" />
    ...
    <td height="30" colspan="3"><table width="466" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="254"><label>
        <?php
		 $profes=mysql_query("SELECT * FROM profesiones where idprofesion=".$row['idprofesion'],$conn) or die(mysql_error());
		  $rowprofes=mysql_fetch_array($profes);
		?>
          <input name="profesion" value="<?php echo $rowprofes['desprofesion'];?>" style="text-transform:uppercase"  type="text" id="nomprofesionesc" size="40"   />
           <input name="nomprofesionesc" value="<?php echo $rowprofes['idprofesion'];?>" style="text-transform:uppercase"  type="hidden" id="nomprofesionesc" size="40"   />
        </label></td>
