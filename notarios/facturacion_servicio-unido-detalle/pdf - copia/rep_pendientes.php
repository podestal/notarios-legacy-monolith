<?php
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'REPORTE DE COMPROBANTES PENDIENTES DE PAGO' ,0,0,'C');
		$this->Ln(15);
		...
		foreach($data as $row)
		{
			...
			$this->CellFitSpace($w[4],6,substr(utf8_decode($row["cliente"]),0,100),'LR',0,'C',$fill);
			$this->Cell($w[5],6,$row["saldo"],'LR',0,'C',$fill);
		...
		foreach($data as $row){
			$sum_saldos = $sum_saldos + $row["saldo"];
		}
		...
		$this->SetXY(225,$height_d);	
		
		$this->Cell(20,6,number_format($sum_saldos, '2', '.', ''),1,0,'C');
	...
	$header = array(utf8_decode('Fec. Emisión'),'Tipo Doc.','Serie','Num. Doc.','Cliente','Saldo','Tip. Mov.');
