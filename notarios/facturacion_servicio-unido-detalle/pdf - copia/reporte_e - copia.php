...
m_regventas.id_regventas AS id,
					...
					d_regventas.`kardex` AS kardex
FROM d_regventas 
INNER JOIN m_regventas ON m_regventas.id_regventas=m_regventas.`id_regventas`
...
AND d_regventas.id_regventas=m_regventas.id_regventas
					";
	if($filtro=="0"){
   ...
   $sql_cuentas =	$sql_cuentas." GROUP BY m_regventas.`id_regventas` ORDER BY m_regventas.fecha desc, m_regventas.tipo_docu asc, m_regventas.factura";
   ...
   $i=0;
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
		...
		foreach($data as $row)
		{
			...
			$this->CellFitSpace($w[5],6,utf8_decode(strtoupper(substr($row["cliente"],0,100))),'LR',0,'C',$fill);
	...
	$header = array(utf8_decode('Fec. Emisión'),'Tipo Doc.','Serie','Num. Doc.','DOC','Cliente','Sub Total','IGV
...
(18%)','Imp.Total');
	...
	$cf =	$cf."";
		...
		$cf =	$cf." and m_regventas.tipo_docu != '04' ";
	}else if($filtro=="2"){
		$cf =	$cf." and m_regventas.tipo_docu = '04' ";
	}
