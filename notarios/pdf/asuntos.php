<?php
	...
	$consulta_asuntos = "SELECT 
						...
						INNER JOIN cliente2 ON cliente2.idcontratante = contratantes.idcontratante WHERE kardex.idtipkar='2'
   ...
   $i=0;
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'INDICE CRONOLOGICO ASUNTOS NO CONTENCIOSOS' ,0,0,'C');
		$this->Ln(15);
		...
		foreach($data as $row)
		{
			$this->Cell($w[0],6,$row["fec_escritura"],'LR',0,'C',$fill);
			$this->Cell($w[1],6,$row["kardex"],'LR',0,'C',$fill);
			$this->Cell($w[2],6,$row["cliente"].$row["empresa"],'LR',0,'C',$fill);
			$this->Cell($w[3],6,$row["contrato"],'LR',0,'C',$fill);
			$this->Cell($w[4],6,$row["escritura"],'LR',0,'C',$fill);
			$this->Cell($w[5],6,$row["minuta"],'LR',0,'C',$fill);
			$this->Cell($w[6],6,$row["folio"],'LR',0,'C',$fill);
			$this->Ln();
			$fill = !$fill;
			$n++;
		}
	...
	$header = array('Fec. Escri.', 'Kardex', 'Contratantes', 'Acto', utf8_decode('Nº Escritura'), utf8_decode('Nº Minuta'),utf8_decode('Nº Folio'));
