<?php
   ...
   $datos_capaces = mysql_query($consulta_capaces, $conexion);	
   
   $i=0;

    while($dato_capaces = mysql_fetch_array($datos_capaces)){
	      $array_capaces[$i] =  $dato_capaces;
	      $i++;
	}

	$total_reg = count($array_capaces);
	...
	for($j=0; $j<$total_reg; $j++){
		...
		${'array'.$contador}[$j] = $array_capaces[$j] ;
	...
	function Header()
	{
		...
		//Título
		$this->Cell(140,10, ' 	INDICE CRONOLOGICO DE CERT. SUPERVIVENCIA - P.CAPAZ' ,0,0,'C');
		//Salto de línea
		$this->Ln(15);
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'INDICE CRONOLOGICO DE CERT. SUPERVIVENCIA - P.CAPAZ' ,0,0,'C');
		$this->Ln(15);
