<?php
	...
	$consulta_vehicular = "SELECT 
							...
							INNER JOIN cliente2 ON cliente2.idcontratante = contratantes.idcontratante WHERE kardex.idtipkar='3'
   ...
   $datos_vehicular = mysql_query($consulta_vehicular, $conexion);	
   
   $i=0;

    while($dato_vehicular = mysql_fetch_array($datos_vehicular)){
	      $array_vehicular[$i] =  $dato_vehicular;
	      $i++;
	}

	$total_reg = count($array_vehicular);
	...
	for($j=0; $j<$total_reg; $j++){
		...
		${'array'.$contador}[$j] = $array_vehicular[$j] ;
	...
	/*function Header()
	{
		...
		//Título
		$this->Cell(140,10, 'INDICE CRONOLOGICO DE TRANSFERENCIAS VEHICULARES' ,0,0,'C');
		//Salto de línea
		$this->Ln(15);
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'INDICE CRONOLOGICO DE TRANSFERENCIAS VEHICULARES' ,0,0,'C');
		$this->Ln(15);
		...
		foreach($data as $row)
		{
			...
			$this->Cell($w[2],6,substr(utf8_decode($row["cliente"]).' '.utf8_decode($row["empresa"]),0,55),'LR',0,'C',$fill);
			$this->Cell($w[3],6,substr(utf8_decode($row["contrato"]),0,51),'LR',0,'C',$fill);
