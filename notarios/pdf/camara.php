<?php
	...
	//echo $consulta_camara;
  
   $datos_camara = mysql_query($consulta_camara, $conexion);	
   
   $i=0;

    while($dato_camara = mysql_fetch_array($datos_camara)){
	      $array_camara[$i] =  $dato_camara;
	      $i++;
	}

	$total_reg = count($array_camara);
	...
	for($j=0; $j<$total_reg; $j++){
		...
		${'array'.$contador}[$j] = $array_camara[$j] ;
	...
	/*function Header()
	{
		...
		//Título
		$this->Cell(140,10, 'INDICE CRONOLOGICO DE CAMARA DE COMERCIO' ,0,0,'C');
		//Salto de línea
		$this->Ln(15);
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'REPORTE PARA LA CAMARA DE COMERCIO' ,0,0,'C');
		$this->Ln(15);
		...
		// Cabecera
		$w = array(10, 20, 20, 20, 25, 50, 50, 25, 65);
		...
		foreach($data as $row)
		{
			...
			$this->Cell($w[1],6,fechabd_an($row["fec_notificacion"]),'LR',0,'C',$fill);
			$this->Cell($w[2],6,fechabd_an($row["fec_constancia"]),'LR',0,'C',$fill);
			$this->Cell($w[3],6,$row["tip_prot"],'LR',0,'C',$fill);
			$this->Cell($w[4],6,$row["importe"],'LR',0,'C',$fill);
			$this->Cell($w[5],6,substr(utf8_decode($row["solicitante"]),0,28),'LR',0,'C',$fill);
			$this->Cell($w[6],6,substr(utf8_decode($row["participante"]),0,28),'LR',0,'C',$fill);
			$this->Cell($w[7],6,$row["dni"],'LR',0,'C',$fill);
			$this->Cell($w[8],6,substr(utf8_decode($row["direccion"]),0,40),'LR',0,'C',$fill);
			$this->Ln();
			$fill = !$fill;
			$n++;
	...
	$header = array('C.P', 'Fec. Not.', 'Fec. Prot.', 'TIT Val', 'Monto', 'Solicitante', 'Aceptante/Aval', 'Ruc/DNI', utf8_decode('Dirección Obligado o Aval'));
