<?php
    ...
    $consulta_libros = "SELECT
						...
						FROM
						libros
						INNER JOIN nlibro ON libros.idnlibro = nlibro.idnlibro
						INNER JOIN tipofolio ON libros.idtipfol = tipofolio.idtipfol
						INNER JOIN tipolibro ON libros.idtiplib = tipolibro.idtiplib
   ...
   $datos_libros = mysql_query($consulta_libros, $conexion);	
   
   $i=0;

    while($dato_libros = mysql_fetch_array($datos_libros)){
	      $array_libros[$i] =  $dato_libros;
	      $i++;
	}

	$total_reg = count($array_libros);
	...
	for($j=0; $j<$total_reg; $j++){
		...
		${'array'.$contador}[$j] = $array_libros[$j] ;
	...
	function Header()
	{
		...
		//Título
		$this->Cell(140,10, 'INDICE CRONOLOGICO LIBROS' ,0,0,'C');
		//Salto de línea
		$this->Ln(15);
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'INDICE CRONOLOGICO LIBROS' ,0,0,'C');
		$this->Ln(15);
		...
		// Cabecera
		$w = array(25, 25, 60, 55, 25, 20, 30, 45);
		...
		foreach($data as $row)
		{
			$this->Cell($w[0],6,$row["num_crono"],'LR',0,'C',$fill);
			$this->Cell($w[1],6,fechabd_an($row["fecha"]),'LR',0,'C',$fill);
			
			
			if(trim($row["empresa"])<>"" and trim($row["cliente"]) <>""){
				$this->Cell($w[2],6,$row["empresa"].'/'.$row["cliente"],'LR',0,'C',$fill);
			}else{
				$this->Cell($w[2],6,substr(utf8_encode($row["empresa"]).utf8_encode($row["cliente"]),0,33),'LR',0,'C',$fill);
			}
			
			$this->Cell($w[3],6,substr($row["tip_lib"],0,32),'LR',0,'C',$fill);
			$this->Cell($w[4],6,$row["n_lib"],'LR',0,'C',$fill);
			$this->Cell($w[5],6,$row["folio"],'LR',0,'C',$fill);
			$this->Cell($w[6],6,$row["tip_fol"],'LR',0,'C',$fill);
			
			if(trim($row["ruc"]) <>""){
				$this->Cell($w[7],6,$row["ruc"],'LR',0,'C',$fill);	
			}else{
				$this->Cell($w[7],6,$row["dni"],'LR',0,'C',$fill);
			}
	...
	$header = array(utf8_decode('N° Cronologico'), 'Fecha', 'Empresa/Cliente', 'Tipo de Libro', utf8_decode('N° de Libro'), utf8_decode('N° de Folio'), 'Tipo de Folio', 'DNI / RUC');
