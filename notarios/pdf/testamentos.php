<?php
   ...
   $datos_testamento = mysql_query($consulta_testamento, $conexion);	
   
   $i=0;

    while($dato_testamento = mysql_fetch_array($datos_testamento)){
	      $array_testamento[$i] =  $dato_testamento;
	      $i++;
	}

	$total_reg = count($array_testamento);
	...
	for($j=0; $j<$total_reg; $j++){
		...
		${'array'.$contador}[$j] = $array_testamento[$j] ;
	...
	/*function Header()
	{
		...
		//Título
		$this->Cell(140,10, 'INDICE CRONOLOGICO DE TESTAMENTOS' ,0,0,'C');
		//Salto de línea
		$this->Ln(15);
	...
	function FancyTable($header, $data, $fechaa, $fechade, $num)
	{
        ...
        $this->Cell(62);
        //Título
        $this->Cell(145,10, 'INDICE CRONOLOGICO DE TESTAMENTOS' ,0,0,'C');
		$this->Ln(15);
