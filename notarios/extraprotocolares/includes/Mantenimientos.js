...
function fgrabarRegistroFirmas()
{
	divResultado = document.getElementById('resul_certi_uno');
	divResultado2 = document.getElementById('confirmaGuarda');
	divResultado2.innerHTML= '<center><img src="../../loading.gif"></center>';	



	var num_certificado=getDataPorId('num_certificado');
	var fecIngreso=getDataPorId('fec_ingreso');
	var estadoRegFirma=getDataPorId("estadoRegFirma");
	var vIdRegistroFirmas=getDataPorId("idRegistroFirmas");
	var arrySolicitante=arryDataSolicitante;


	//var fileRf = document.getElementById("fileRf");
	//var file = fileRf.files[0];
	var data = new FormData();

	data.append("arrFiles",JSON.stringify(arrFiles));

	data.append("fecIngreso",fecIngreso);
	data.append("numCertificado",num_certificado);
	data.append("arrySolicitante",JSON.stringify(arrySolicitante));
	data.append("estadoRegFirma",estadoRegFirma);
	data.append("idRegistroFirmas",vIdRegistroFirmas);
	
	
	$.ajax({
		url:'../model/guardaRegistroFirmas.php',
		data:data,
		cache:false,
		contentType:false,
		processData:false,
	//	dataType:'json',
		...
		success:function(response){	
			...
			var vIdRegistroFirmas=objResult.idRegistroFirmas;
			var vNumControl=objResult.numControl;
			document.getElementById('idRegistroFirmas').value=vIdRegistroFirmas;
			document.getElementById('num_certificado').value=getValorLlenoCeros(5,vNumControl);
	...
	ajax.open("POST", "../model/guardaRegistroFirmas.php",true);	
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {

					divResultado.innerHTML='';

			var objResult= JSON.parse(ajax.responseText);
			var vIdRegistroFirmas=objResult.idRegistroFirmas;
			var vNumControl=objResult.numControl;
			document.getElementById('idRegistroFirmas').value=vIdRegistroFirmas;
			document.getElementById('num_certificado').value=getValorLlenoCeros(5,vNumControl);
	...
	ajax.send(data+"&nombre="+"giancarlo Ramos Rivas");
	*/
...
function getValorLlenoCeros(vLength,valor)
			{
				var vvvLengthValor=valor.toString().length;
				if(vLength>vvvLengthValor)
				{
					
					
					var totalcadena=vLength-vvvLengthValor;
					var i=0;
					var ceros="";
							while(totalcadena>i)
							{
...
function fgrabarLegalizacion()
{
	...
	var tipdocSolic=getDataPorId('tipdoc_solic');
	var numdocSolic=getDataPorId('numdoc_solic');
	var motivoSolic=getDataPorId('motivo_solic');
	var textoCuerpo=getDataPorId('texto_cuerpo');
	var fecIngreso=getDataPorId('fec_ingreso');
	var idLegalizacion=getDataPorId('idLegalizacion');
	var arrySolicitante=arryDataSolicitante;
	...
	ajax.open("POST", "../model/guardaLegalizacion.php",true);	
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
	...
	ajax.send("direCertificado="+direCertificado+"&tipdocSolic="+tipdocSolic+"&numdocSolic="+numdocSolic+"&motivoSolic="+
	motivoSolic+"&textoCuerpo="+textoCuerpo+"&fecIngreso="+fecIngreso+"&arrySolicitante="+JSON.stringify(arrySolicitante)+"&idLegalizacion="+idLegalizacion);
...
{	
	...
	var _dni_destinatario = document.getElementById('numdoc2').value;
	var _nom_destinatario = document.getElementById('destinatario2').value;
	var _dir_destinatario = document.getElementById('dirdestino2').value;
	...
	var _fec_recogio       = '';
	var _fact_recogio      = '';
	var _recepcion  = document.getElementById('recepcion').value;
	var _firmo  = document.getElementById('firmo').value;
...
{	
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	// ajax.send("num_carta="+_num_carta+"&fec_ingreso="+_fec_ingreso+"&id_remitente="+_id_remitente);
	ajax.send("num_carta="+_num_carta+"&fec_ingreso="+_fec_ingreso+"&id_remitente="+_id_remitente+"&remitente="+_remitente+"&direccion_remi="+_direccion_remi+"&telefono="+_telefono);
...
{	
	...
	ajax.open("POST", "../model/editLegalizacion.php",true);	
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function fbussello2()
{
	...
	ajax.open("POST","listdetallecarta2.php",true);
	ajax.onreadystatechange=function() {
		if ((ajax.readyState==4 && ajax.status==200)) {
			divResultado.innerHTML = ajax.responseText;
			//fcerrardivedicion();
...
{	
	...
	var _fecha_desde = document.getElementById('fecha_desde').value;
	var _fecha_hasta = document.getElementById('fecha_hasta').value;
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("num_kardex="+_num_kardex+"&asunto="+_asunto+"&fec_ingreso="+_fec_ingreso+"&nom_recep="+_nom_recep+"&hora_recep="+_hora_recep+"&referencia="+_referencia+"&nom_comu="+_nom_comu+"&tel_comu="+_tel_comu+"&email_comu="+_email_comu+"&documento="+_documento+"&num_crono="+_num_crono+"&fecha_crono="+_fecha_crono+"&num_formu="+_num_formu+"&lugar_formu="+_lugar_formu+"&observacion="+_observacion+"&id_viaje="+_id_viaje+"&via="+_via+"&fecha_desde="+_fecha_desde+"&fecha_hasta="+_fecha_hasta);
...
function fgrabcertidomic()
{
	...
	if(document.getElementById('nomprofesionesc')){
		var _nomprofesionesc	 = document.getElementById('nomprofesionesc').value;
	}
	if(document.getElementById('nomprofesionesc')){
		var _idprofesionc	 = document.getElementById('hdnnomprofesionesc').value;
	}
	
	

	let _fecha_ocupa = document.getElementById('fecha_ocupa').value;
	let _declara_ser = document.getElementById('declara_ser').value;
	let _propietario = document.getElementById('propietario').value;
	let _recibido = document.getElementById('recibido').value;
	let _recibo_empresa = document.getElementById('recibo_empresa').value;
	let _numero_recibo = document.getElementById('numero_recibo').value;
	let _mes_facturado = document.getElementById('mes_facturado').value;
	...
	_tdoc_testigo+"&ndocu_testigodom="+_ndocu_testigodom+"&sexo="+sexo+"&idestcivil="+idestcivil+"&nomprofesionesc="+_nomprofesionesc+"&idprofesionc="+_idprofesionc+"&fecha_ocupa="+_fecha_ocupa+"&declara_ser="+_declara_ser+"&propietario="+_propietario+"&recibido="+_recibido+"&recibo_empresa="+_recibo_empresa+"&numero_recibo="+_numero_recibo+"&mes_facturado="+_mes_facturado);
...
{	
	...
	if(document.getElementById('nomprofesionesc2')){
	...
	ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
	//enviando los valores
	ajax.send("num_certificado="+_num_certificado+"&fec_ingreso="+_fec_ingreso+"&num_formu="+_num_formu+"&nombre_solic="+_nombre_solic+"&tipdoc_solic="+_tipdoc_solic+"&numdoc_solic="+_numdoc_solic+"&domic_solic="+_domic_solic+"&motivo_solic="+_motivo_solic+"&distrito_solicc="+_distrito_solic+"&texto_cuerpo="+_texto_cuerpo+"&justifi_cuerpo="+_justifi_cuerpo+"&id_domiciliario="+_id_domiciliario+"&nom_testigo="+_nom_testigo+"&tdoc_testigo="+
	_tdoc_testigo+"&ndocu_testigodom="+_ndocu_testigodom+"&sexo="+sexo+"&idestcivill="+idestcivil+"&nomprofesionesc2="+_nomprofesionesc+"&idprofesionc2="+_idprofesionc+"&fecha_ocupa="+_fecha_ocupa+"&declara_ser="+_declara_ser+"&propietario="+_propietario+"&recibido="+_recibido+"&recibo_empresa="+_recibo_empresa+"&numero_recibo="+_numero_recibo+"&mes_facturado="+_mes_facturado);
