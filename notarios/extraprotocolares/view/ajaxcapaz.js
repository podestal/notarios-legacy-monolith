...
function buscar_cliente_cc(){
	...
	var tipodoc=document.getElementById("tipdocu").value;
	var numdoc=document.getElementById("numdocu").value;
	
	ajax=objetoAjax();

	ajax.open("POST", "buscar_clientes_capaz.php",true);	
...
function grabarcliente_cc()
{
	...
	//datoscliente.php
	ajax.open("POST","grabar_cliente_capaz.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function grabarcliente2_cc()
{
	...
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","grabar_cliente_capaz2.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
			//mostrar resultados en esta capa
			divResultado.innerHTML = ajax.responseText;
