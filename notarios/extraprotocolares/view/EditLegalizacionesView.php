...
$numcarta  = $_REQUEST['numfirma'];	
// print_r($numcarta);return false;
...
$consulcartas = mysql_query("SELECT l.*, DATE_FORMAT(STR_TO_DATE(l.fec_ingreso,'%d/%m/%Y'),'%d/%m/%Y') AS fec_ingreso2 FROM legalizaciones as l WHERE l.num_firma='$numcarta'", $conn) or die(mysql_error());
$rowcarta = mysql_fetch_array($consulcartas);

$numca = $rowcarta['id_parte'];
// print_r($numca);return false;
...
//AjaxReturn('../../reportes_word/generador_permiviaje_interior.php?id_viaje='+_id_viaje+'&usuario_imprime='+_usuario_imprime+'&nom_notario='+_nom_notario);
		window.open("genera_legalizacion.php?numcarta="+_numcarta+"&usuario_imprime="+_usuario_imprime+"&nom_notario="+_nom_notario);
...
function actualizarContenido(){
    ...
    let contenidoCarta = document.getElementById('contecarta');
    let fechaentrega = document.getElementById('fecentrega').value;
    let fechaFormateada = fechaentrega.split('/').reverse().join('/');;
    ...
    let newContenidoCarta = contenidoCarta.value.replace('<F>',fechaLetras).replace('<F_N>',fechaentrega).replace('<HORA>',horaentrega.value).replace('<DIRECCION>',dirdestino2.value)
    
    contenidoCarta.value=newContenidoCarta
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td colspan="2"><table  width="100%">
      <tr>
        <td width="14%"><span class="camposss">Nro firma:</span></td>
        <td width="16%"><input name="numcarta" type="hidden" id="numcarta" value="<?php echo $rowcarta['num_firma']; ?>" style="text-transform:uppercase" size="15" />
  ...
  <tr>
    <td colspan="2">
    <fieldset>
    <legend><span class="camposss">Parte</span></legend>
    <table  width="100%">
    <tr>
        ...
        <tr>
          <td width="65%"><span class="camposss">N. Documento :
            <input name="numdoc" type="text" id="numdoc" style="text-transform:uppercase" onKeyPress="send(event);" value="<?php echo $rowcarta['id_parte']; ?>" size="16" maxlength="15" />
          </span></td>
          <td width="35%"><!--<a href="#" onClick="newParticipante();//fShowDatosProveeClick()"> <img src="../../images/search.png" alt="" width="15" height="15" border="0" /></a>-->
            ...
            <input name="telefono" type="text" id="telefono" onKeyPress="return solonumeros(event)" value="<?php echo $rowcarta['telefono']; ?>" style="text-transform:uppercase" size="15" maxlength="20"  /></td>

        </tr>
        <td><div id="rpta_bus">
          <table  width="100%" border="0" cellpadding="0" cellspacing="0">
              <tr>
          <td colspan="3"><span class="camposss">Remitente:</span></td><td><input name="remitente2" type="text" id="remitente2" onKeyUp="remitente1();"  style="text-transform:uppercase" onkeypress="return soloLetrasynumeros(event)" value="<?php $c_desc = $rowcarta['nombre'];
          ...
          <input type="hidden"  name="remitente" id="remitente"  value="<?php echo strtoupper($rowcarta['nombre']); ?>"/>          </td>
          </tr>
        <tr>
          <td><span class="camposss">Direccion:</span> </td>
          <td colspan="3"><input name="direccion_remi1" type="text" id="direccion_remi1"   onKeyUp="direccionremi1();" style="text-transform:uppercase" value="<?php $c_desc = $rowcarta['direccion'];
		  $textorpat=str_replace("?","'",$c_desc);
		  $textoamperpat=str_replace("*","&",$textorpat);
          ...
          <input type="hidden"  name="direccion_remi" id="direccion_remi" value="<?php  echo strtoupper($rowcarta['direccion']); ?>" /> </td>
         </tr>
            </table> 
        </div>
        </td>
    ...
    <table  width="100%">
		...
		<tr>
          ...
          <td><label for="select"></label>
            ...
            <?php 
			while($rowdocw2=mysql_fetch_array($sqlrctm2)){
				if($rowdocw2['idtipdoc']=='1'){echo "<option value='".$rowdocw2['idtipdoc']."' selected='selected'>".$rowdocw2['destipdoc']."</option>";}
				else{echo "<option value='".$rowdocw2['idtipdoc']."'>".$rowdocw2['destipdoc']."</option>";}
          ...
          <td width="37%"><input name="numdoc2" type="text" id="numdoc2" style="text-transform:uppercase" onKeyPress="send2(event);" value="" size="16" maxlength="15" />&nbsp;
...
<div id="clientenewdni" class="dalib" style="display:none; z-index:7;">
  ...
  <tr>
    ...
    <td width="119" height="30" align="right"><span class="camposss">Apellido Maternos :</span></td>
