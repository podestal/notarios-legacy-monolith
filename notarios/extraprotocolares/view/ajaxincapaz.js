...
function buscar_cliente_cc(){
	
	var divResultado = document.getElementById('cambio_caract');
	var tipodoc=document.getElementById("tipdocu_rep").value;
	var numdoc=document.getElementById("numdocu_rep").value;
	
	ajax=objetoAjax();

	ajax.open("POST", "buscar_clientes_incapaz.php",true);	
	...
	ajax.send("tipodoc="+tipodoc+"&numdoc="+numdoc);

}

function grabarcliente_cc()
{
	//donde se mostrará el resultado
	divResultado = document.getElementById('cambio_caract');
	//tomamos el valor de la lista desplegable
	tipoper="N";
	numdoc_solic=document.getElementById('num_docu').value;
	...
	//datoscliente.php
	ajax.open("POST","grabar_cliente_incapaz.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function grabarcliente2_cc()
{
	...
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","grabar_cliente_incapaz2.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
			//mostrar resultados en esta capa
			divResultado.innerHTML = ajax.responseText;
