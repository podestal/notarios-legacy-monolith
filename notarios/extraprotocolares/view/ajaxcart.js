...
function buscar_cliente_car(){
	...
	var _num_carta      = document.getElementById('tipdocumento').value;
	var _num_doc     = document.getElementById('numdoc').value;
	ajax=objetoAjax();

	ajax.open("POST", "buscar_clientes_car.php",true);	
	...
	ajax.send("_num_carta="+_num_carta+"&_num_doc="+_num_doc);

}

function buscar_cliente_car2(){
	
	var divResultado = document.getElementById('rpta_bus2');
	var _num_carta      = document.getElementById('tipdocumento2').value;
	var _num_doc     = document.getElementById('numdoc2').value;
	ajax=objetoAjax();

	ajax.open("POST", "buscar_clientes_car2.php",true);	
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) { 
			divResultado.innerHTML = ajax.responseText;		
		}
...
function grabarcliente_car()
{
	...
	//datoscliente.php
	ajax.open("POST","grabar_cliente_car.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function grabarcliente2_dom()
{
	...
	numdoc_solic=document.getElementById('numdoc').value;
	...
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","grabar_cliente_car2.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
			//mostrar resultados en esta capa
			divResultado.innerHTML = ajax.responseText;
