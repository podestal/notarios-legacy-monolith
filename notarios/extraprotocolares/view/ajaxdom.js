...
function buscar_cliente_car(){
	...
	var _tipo_doc     = document.getElementById('tipdoc_solic').value;
	var _num_doc     = document.getElementById('numdoc_solic').value;
	
	ajax=objetoAjax();

	ajax.open("POST", "buscar_clientes_dom.php",true);	
...
function editarcliente_dom(){
	...
	idCliente=document.getElementById('hdnIdCliente').value;
	//instanciamos el objetoAjax
	ajax=objetoAjax();
	//usamos el medoto POST
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","editar_cliente_dom.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
...
function grabarcliente_dom()
{
	...
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","grabar_cliente_dom.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
			//mostrar resultados en esta capa
			divResultado.innerHTML = ajax.responseText;
...
function grabarcliente2_dom()
{
	...
	//archivo que realizará la operacion
	//datoscliente.php
	ajax.open("POST","grabar_cliente_dom2.php",true);
	ajax.onreadystatechange=function() {
		if (ajax.readyState==4 && ajax.status==200) {
			//mostrar resultados en esta capa
			divResultado.innerHTML = ajax.responseText;
