...
$numdoc  = $_POST['numdoc'];
$tipdoc = $_POST['tipdoc_solic'];
$tipper= $_POST['tipoPer'];

$consulta="";
if($tipper=="N"){
	$consulta="select nombre,numdoc, direccion,idtipdoc,telcel from cliente where  numdoc = '$numdoc' and idtipdoc='$tipdoc' and tipper='$tipper'";
}else if($tipper=="J"){
	$consulta="select razonsocial,numdoc, domfiscal,idtipdoc,telempresa from cliente where  numdoc = '$numdoc' and idtipdoc='$tipdoc' and tipper='$tipper'";
}
$sqlclie=mysql_query($consulta, $conn) OR die(mysql_error());

$row=mysql_fetch_row($sqlclie);
...
<form name="frmbuscakardex" id="frmbuscakardex" method="post" action="">
  ...
  <tr>
    ...
    <table  width="100%">
        <tr>
          ...
          <td width="15%"><input name="numdoc" type="text" id="numdoc" style="text-transform:uppercase" value="<?php echo $row['1'];?>" onkeypress="return solonumeros(event)" size="15" maxlength="15"/>&nbsp;
          ...
          <td width="23%" colspan="-1"><select name="tipdoc_solic" style="camposss">
<?php 

$sqlta="select destipdoc from tipodocumento where idtipdoc='".$row['3']."'";
			$rsta=mysql_query($sqlta);
			$filata=mysql_fetch_row($rsta);
echo "<option value=".$row['idtipdoc'].">".$filata[0]."</option>";?> 
</select>
</td>

          <td width="24%"><span class="camposss">Telefono:</span></td>

          <td width="16%"><input name="telefono" type="text" id="telefono"  style="text-transform:uppercase" size="15" maxlength="15" value="<?php echo $row['4'];?>" onkeypress="return solonumeros(event)"/></td>
        </tr>
        <tr>
          <td><span class="camposss">Remitente:</span> </td>
          <td colspan="3"><input name="remitente2" type="text" id="remitente2" style="text-transform:uppercase" size="60" onkeypress="return soloLetrasynumeros(event)" onKeyUp="remitente1();" value="<?php echo $row['0'];?>" maxlength="400"  />
          <input type="hidden" name="remitente" id="remitente" >
          </td>
          </tr>
        <tr>
          <td><span class="camposss">Direccion:</span> </td>
          <td colspan="3"><input name="direccion_remi1" style="text-transform:uppercase" type="text" id="direccion_remi1" size="60" onkeypress="return soloLetrasynumeros(event)" value="<?php echo $row['2'];?>"  onKeyUp="direccion_remi2();" maxlength="300"/>
...
<?php
		  	...
		  	if($tipper=="N"){
				header ("Location : addContratanteN.php?numdoc=".$numdoc."&tipdoc=".$tipdoc."&tipper=".$tipper."");
			}else if($tipper=="J"){
				header ("Location : addContratanteJ.php?numdoc=".$numdoc."&tipdoc=".$tipdoc."&tipper=".$tipper."");
			}
	    }
