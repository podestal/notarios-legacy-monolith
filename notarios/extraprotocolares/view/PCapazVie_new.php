...
echo "<select name='tipdocu'  id='tipdocu' onchange='validacion();' class='camposss' style='width:200'>";
			$combo1 = mysql_query( "SELECT tipodocumento.codtipdoc AS 'id', tipodocumento.destipdoc AS 'des'
FROM tipodocumento
ORDER BY tipodocumento.destipdoc ASC",$conn);
			while ($rs1=mysql_fetch_assoc($combo1)){
			if($rs1['id']=='01'){
				echo "<option value='".$rs1['id']."' selected='selected'>".$rs1['des']."</option>";
			}
			echo "<option value='".$rs1['id']."'>".$rs1['des']."</option>";
			}
			echo "</select>";

?></td>
          <td colspan="-1" align="right"><span class="camposss">Nro.</span></td>
          <td colspan="-1"><input  name="numdocu" type="text" id="numdocu" style="text-transform:uppercase" size="15" maxlength="20" placeholder="N. documento"  onkeypress="return solonumeros(event)"/></td>
        </tr>
        <tr>
          <td colspan="4"><input name="nombre1" type="text" id="nombre1" style="text-transform:uppercase" size="100" maxlength="400" onKeyUp="editnombre();"  onkeypress="return soloLetras(event)"/>
      ...
      <tr>
        ...
        <tr>
          <td colspan="4"><span class="camposss">y domiciliar en:    </span>
            <input name="ndireccion" type="text" id="ndireccion" style="text-transform:uppercase" size="84" maxlength="1000" placeholder="direccion" onKeyUp="editdireccion();"/><input type="hidden" name="direccion" style="text-transform:uppercase" id="direccion" /></td>
        </tr>
