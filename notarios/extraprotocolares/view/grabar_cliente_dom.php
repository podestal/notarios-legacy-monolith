...
$queryIdCliente = "SELECT idcliente FROM cliente order by idcliente desc limit 1";
$resultIdCliente = mysql_query($queryIdCliente,$conn) or die(mysql_error());
$rowIdCliente = mysql_fetch_assoc($resultIdCliente);
...
<!-- aca empieza la webada-->
	<table>
        <tr>
          <input type="hidden" name="idGrabarClienteSolicitante" id="idGrabarClienteSolicitante" value="<?php echo $rowIdCliente['idcliente'];?>">
          <td width="14%"><span class="camposss">Solicitante:</span></td>
          <td colspan="3"><input name="nnombre_solic" type="text" id="nnombre_solic" style="text-transform:uppercase" size="100" maxlength="400" onkeypress="return soloLetras(event)" value="<?php echo $apepat." ".$apemat.", ".$prinom." ".$segnom;?>"/>
            <input type="hidden" name="nombre_solic" value="<?php echo $apepat." ".$apemat.", ".$prinom." ".$segnom;?>" style="text-transform:uppercase" id="nombre_solic" />
            </td>
        </tr>
      
        <tr>
          <td><span class="camposss">Domicilio:</span></td>
          <td colspan="3"><input name="domic_solic" type="text" id="domic_solic" style="text-transform:uppercase" size="100" maxlength="2000"  placeholder="Direccion" onKeyUp="domicilio();" value="<?php echo $direccion;?>"/>
		...
		?>
          ...
          <td colspan="3"><input name="distrito_solicc" value="<?php echo $res1['coddis'];?>" type="hidden" id="distrito_solicc" size="15" />
      ...
      <tr>
        <td width="254"><label>
          <input name="profesion" value="<?php echo $profocupa;?>" style="text-transform:uppercase"  type="text" id="nomprofesionesc" size="40"   />
           <input name="nomprofesionesc" value="<?php echo $idprofesion;?>" style="text-transform:uppercase"  type="hidden" id="hdnnomprofesionesc" size="40"   />
        </label></td>
