...
$consulpermiviaje = mysql_query("SELECT permi_viaje.*, 
	DATE_FORMAT(permi_viaje.fec_ingreso,'%d/%m/%Y') as 'eval_fecingreso', 
	DATE_FORMAT(permi_viaje.fecha_crono,'%d/%m/%Y') AS 'eval_fechacrono' 
	FROM permi_viaje
WHERE permi_viaje.id_viaje='$id_viaje'", $conn) or die(mysql_error());
...
$(document).ready(function(){ 
	...
	.dialog({
					...
					{id: "btnqr", text: "Generar QR",click: function() {fImprimirqr();}},
...
<input name="idasunto" type="hidden" id="idasunto" style="text-transform:uppercase" value="<?php echo $rowcpermiso['asunto']; ?>" size="2" />&nbsp;<input name="desasunto" type="hidden" id="desasunto" style="text-transform:uppercase" value="<?php echo $rowcpermiso['asunto']; ?>" size="7" /> <input name="asunto" type="hidden" id="asunto" style="text-transform:uppercase" value="<?php echo $rowcpermiso['asunto']; ?>" size="40" /></td>
        ...
        <tr>
          ...
          <td width="10%"><span class="camposss">Motivo:</span> </td>
        ...
        <tr>
		  ...
		  <?php 
		  		$arrVias = array('TERRESTRE', 'AEREA', 'MARITIMA', 'AEREA - TERRESTRE');
		  		// echo '<option value="'.$rowcpermiso["via"].'" selected>'.$rowcpermiso['via'];'</option>'
				
				foreach($arrVias as $via){
					if($rowcpermiso["via"] != $via){
						echo '<option value="'.$via.'">'.$via.'</option>';
					}else{
						echo '<option value="'.$rowcpermiso["via"].'" selected>'.$rowcpermiso['via'].'</option>';
					}
				}
        ...
        <tr>
		  ...
		  <a href="#" onClick="fmuescontenidocartas()"><img src="../../images/help.png" alt="" width="12" height="12" border="0" /></a>
		  
		  </span></td>
		  
          <td colspan="3"><textarea onkeypress="return maxLengthX(event,this,1000);" name="observacionG" style="text-transform:uppercase;" id="observacionG" cols="100" rows="3"  onKeyUp="return soloLetras(event)"><?php 
	  
	  $c_desc22qw=$rowcpermiso['observacion'];
