...
function EditPartiP2(_id_poder, _id_contrata,_anio)
	{

		$('<div id="div_editcontra" title="div_editcontra"></div>').load('EditParticipantesp2.php?id_poder='+_id_poder+'&id_contrata='+_id_contrata+'&anio='+_anio)
		.dialog({
						autoOpen: true,
						position :["center","top"],
						width   : 500,
...
// GUARDA NUEVO CONTRATANTE PARA EL PODER.
	...
	{
		var _id_prote = document.getElementById('id_prote2').value;
		var _cod_tipop = document.getElementById('cod_tipop').value
		var _id_contrata = "";
		var _anio = document.getElementById('id_prote2').value;
		
		$('<div id="div_newcontra" title="div_newcontra"></div>').load('NewParticipanteP2.php?id_prote='+_id_prote+'&id_contrata='+_id_contrata+'&cod_tipop='+_cod_tipop+'&anio='+_anio)
	.dialog({
					autoOpen: true,
					position :["center","top"],
					width   : 750,
	...
	function ElimContratante(_id_poder2, _id_contrata2, _anio)
	{
		...
		{
			felimContratante2(_id_poder, _id_contrata,_anio);
		}	
	}
...
<table width="780" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td ><?php
				...
				$oBarra->Show()  						    ; 
				?>
      <input name="id_prote2" type="hidden" id="id_prote2"  value="<?php echo $id_prote; ?>"  />
...
$consulcontrat=mysql_query("SELECT protesto_participantes.id_protesto, protesto_participantes.id_participante, protesto_participantes.num_docparti,protesto_participantes.descri_parti,
 c_protesto.des_condicionp,protesto_participantes.anio
...
echo "<table width='100%' border='1' cellpadding='0' cellspacing='0' bordercolor='#333333' >
  <tr>
	...
	<td width='16' align='center' ><span class='reskar'><a href='#' id='".$row['id_protesto']."' name='".$row['id_participante']."' anio='".$row['anio']."' onclick='EditPartiP2(this.id,this.name,$anio)'><img src='iconos/editamv.png' width='16' height='18'></a></span></td>
	<td width='16' align='center' ><span class='reskar'><a href='#' id='".$row['id_protesto']."' name='".$row['id_participante']."' anio='".$row['anio']."' onclick='ElimContratante(this.id,this.name,$anio)'><img src='iconos/eliminamv.png' width='16' height='18'></a></span></td>
	<td width='16' align='center' ><span class='reskar'><a href='#' id='".$row['id_protesto']."' name='".$row['id_participante']."' anio='".$row['anio']."' onclick='fNotificacion(this.id,this.name,$anio)'><img src='iconos/formu.png' width='16' height='18'></a></span></td>
  </tr>
</table>";
$i++;
}
