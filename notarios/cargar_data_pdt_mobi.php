...
$sqlkardex = "SELECT DISTINCT kardex.codactos, kardex.idkardex, kardex.kardex, kardex.idtipkar, CAST(FN_ONLYNUM(kardex.numescritura) AS UNSIGNED) numescritura, kardex.fechaescritura, kardex.fechaconclusion
FROM kardex 
WHERE kardex.idtipkar='4' AND (kardex.numescritura>=".$fechade." AND kardex.numescritura<=".$fechaha.")
AND kardex.fechaconclusion<>'' AND kardex.fechaconclusion LIKE '%".$anio."%'
ORDER BY numescritura ASC";
...
while($row = mysql_fetch_array($result)) {
	...
	while ($i < count ($actoss) ) {
        ...
        $consulta=mysql_query("Select * from tiposdeacto where idtipoacto = '$numacto' and actosunat <> '' AND (actosunat = '01' OR actosunat = '02' OR actosunat = '03' 
								...
								OR actosunat = '24' OR actosunat = '25' OR actosunat = '26')", $conn) or die(mysql_error());
