...
function buscar_asuntos(page){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		new Ajax.Updater("lst_asuntos", "consultas/alfabeticos/asuntos.php?" + campos+"&pag=" + page);	
	}
...
function pdf_asuntos(){
	...
	if(flag==1){
		var campos =$("frmescri").serialize();
		window.open('pdf_crono/alfabetico/pdf_crono_asuntos.php?'+campos);
	}
