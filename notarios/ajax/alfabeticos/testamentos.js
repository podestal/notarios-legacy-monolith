...
function buscar_testamentos(page){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		new Ajax.Updater("lst_testamentos", "consultas/alfabeticos/testamentos.php?" + campos+"&pag=" + page);	
	}
...
function pdf_testamentos(){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		window.open('pdf_crono/alfabetico/pdf_crono_testamentos.php?'+campos);
	}
