...
function buscar_escrituras(page){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		new Ajax.Updater("lst_escrituras", "consultas/alfabeticos/escrituras.php?" + campos+"&pag=" + page);	
	}
...
function pdf_escrituras(){
	...
	if(flag==1){
		var campos =$("frmescri").serialize();
		window.open('pdf_crono/alfabetico/pdf_crono_escrituras.php?'+campos);
	}
