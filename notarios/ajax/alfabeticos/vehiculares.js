...
function buscar_vehiculares(page){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		new Ajax.Updater("lst_vehiculares", "consultas/alfabeticos/vehiculares.php?" + campos+"&pag=" + page);	
	}
...
function pdf_vehiculares(){
	
	var flag = validar_fechas();
	
	if(flag==1){
		var campos =$("frmescri").serialize();
		window.open('pdf_crono/alfabetico/pdf_crono_vehiculares.php?'+campos);
	}
