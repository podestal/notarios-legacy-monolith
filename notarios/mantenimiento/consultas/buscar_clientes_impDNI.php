...
if($numdoc!=""){
	...
	if (!empty($row) ){
		 if ($row['tipper']=="N" and $row['numdoc']!=""){
			 include("mostrarcliente1.php");
			 
			}else{ 
				if ($row['tipper']=="N" and $row['numdoc']==""){
			 echo"<span style='font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 16px;'>No se encontro al cliente :</span><input name='nuevo' type='button' onclick='newclient2();newclient()' value='Nuevo Cliente' /><input name='cancelar' type='button' onclick='regresa_caja();' value='Cancelar' />";
				  }
			  } 
	   ...
	   if ($row['tipper']=="J" and $row['numdoc']!=""){
			 include ("mostrarcliente1.php");
				...
				if ($row['tipper']=="J" and $row['numdoc']==""){
			 echo"<span style='font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 16px;'>No se encontro la empresa :</span><input name='cancelar2' type='button' onclick='newclientempresa2();newclientempresa()' value='Nueva Empresa' /><input name='cancelar' type='button' onclick='regresa_caja();' value='Cancelar' />"; 
				  }
