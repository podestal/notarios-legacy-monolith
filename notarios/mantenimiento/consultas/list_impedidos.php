	...
	$sql_cliente = "select	
					...
					CONCAT(cliente.apepat,' ',cliente.apemat,' ',cliente.prinom,' ',cliente.segnom) as nombre,
	...
	if($tip_persona<>"" and $doic=="" and $cliente=="" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona'";
	}
	
	if($tip_persona=="" and $doic<>"" and $cliente=="" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.numdoc like '%$doic%'";
	}
	
	if($tip_persona=="" and $doic=="" and $cliente<>"" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%'";
	}
		
	if($tip_persona=="" and $doic=="" and $cliente=="" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.tipocli='$tip_cliente'";
	}
	...
	if($tip_persona<>"" and $doic<>"" and $cliente=="" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and cliente.numdoc like '%$doic%'";
	}
	
	if($tip_persona<>"" and $doic=="" and $cliente<>"" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%')";
	}
	
	if($tip_persona<>"" and $doic=="" and $cliente=="" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and cliente.tipocli='$tip_cliente'";
	}
	
	if($tip_persona=="" and $doic<>"" and $cliente<>"" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.numdoc like '%$doic%' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%')";
	}
	
	if($tip_persona=="" and $doic<>"" and $cliente=="" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.numdoc like '%$doic%' and cliente.tipocli='$tip_cliente'";
	}
	
	if($tip_persona=="" and $doic=="" and $cliente<>"" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%') and cliente.tipocli='$tip_cliente'";
	}
	...
	if($tip_persona<>"" and $doic<>"" and $cliente<>"" and $tip_cliente==""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and cliente.numdoc like '%$doic%' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%')";
	}
	
	if($tip_persona<>"" and $doic<>"" and $cliente=="" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and cliente.numdoc like '%$doic%' and cliente.tipocli='$tip_cliente'";
	}
	
	if($tip_persona<>"" and $doic=="" and $cliente<>"" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%') and cliente.tipocli='$tip_cliente'";
	}
	
	if($tip_persona=="" and $doic<>"" and $cliente<>"" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.numdoc like '%$doic%' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%') and cliente.tipocli='$tip_cliente'";
	}
	...
	if($tip_persona<>"" and $doic<>"" and $cliente<>"" and $tip_cliente<>""){
		$sql_cliente =	$sql_cliente." and cliente.tipper='$tip_persona' and cliente.numdoc like '%$doic%' and (cliente.nombre like '%$cliente%' or cliente.razonsocial like '%$cliente%') and cliente.tipocli='$tip_cliente'";
	}
    ...
    while($cliente = mysql_fetch_array($res_cliente)){
		...
		$arr_cliente[$i][8] = $cliente["impeorigen"]; 
		$i++; 
    }
	...
	<table width="870" border="1" cellspacing="0" cellpadding="0" >
  		  <tr style="background-color:#CCCCCC">
              ...
              <td width="60" align="center"><span class="titubuskar0">Oficio</span></td>
		  ...
		  for($j=0; $j<count($arr_cliente); $j++){ ?>
         	 <tr id="fila<?php echo $arr_cliente[$j][0]; ?>" >
				...
				<td align='center'>
                	<span class='reskar'><?php echo holaacentos($arr_cliente[$j][2].$arr_cliente[$j][3]); ?></span>
                </td>
                <td align='center'>
                ...
                <td align='center'>
					...
					if($arr_cliente[$j][8]!=""){
						 echo strtoupper($arr_cliente[$j][8]); 
					}else{
						
						$c2=mysql_query("SELECT impedidos.`motivo` FROM deta_impe
INNER JOIN cliente ON cliente.idcliente=deta_impe.`idcliente`
INNER JOIN impedidos ON deta_impe.`idimpedido`=`impedidos`.`idimpedido`
WHERE deta_impe.`idcliente`='".$arr_cliente[$j][0]."'",$conexion);
					
						$resc2=mysql_fetch_array($c2);
						echo strtoupper($resc2[0]);
					}
                ...
                <td align='center'>
                    ...
                    <img src="../../images/pencil.png"  height="18" width="18" title="Editar" style="cursor:pointer" onclick="modificar_impedido('<?php echo $arr_cliente[$j][0]; ?>')"/>
                    </span>
                </td>
          ...
          <tr height='25'>
                <td colspan='9' align='center' valign='bottom'>
                    <table style='margin-bottom:4px'>
                       <tr class='paginacion'>
                        <?php if($pag>7){?>
                            <td><div class='pagina' style='cursor:pointer; width:14px' title='Regresar' onclick="listar_impedidos('<?php echo ($ini_pag-1); ?>')"><--</div></td>
                        <?php } 
                        ...
                        <td><div class='pagina' style='cursor:pointer; width:14px' title='Continuar' onclick="listar_impedidos('<?php echo ($ini_pag+7); ?>')">--></div></td>
                        <?php
                        }
                        ?>	  
