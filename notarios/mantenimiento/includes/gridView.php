		...
		function Show()
			{
				...
				*/
				...
				$this->js = ' <style>
								.'.$this->name.' {
								background-color: WHITE;font-size: 10px;border-collapse: collapse;}
								.'.$this->name.' .header td {
				...
				foreach($fields as $xfield)
					{
						...
						if($num<=$this->NumFields && $this->NumFields>0)
							{
								$th = $th.'<td bgcolor="#CCCCCC" class="'.$this->cssCab.'" id="C'.$columns.'">'.$xfield->name.'</td>'; //  para cambiar el color de las cabeceras = bgcolor="#CCCCCC"
							}
						$num++;
						}
						$columns++;
					...
					if($this->botonModi!=""){
					$th = $th.'<td bgcolor="#CCCCCC" class="'.$this->cssCab.'" colspan="2">Accion</td>';
					}
				while ($row = $this->qRs->fetch_array(MYSQLI_BOTH))
					{
						...
						$td = $td.'<td style="display:none;"><input type="hidden" id="datos'.$numFil.'" size="1" name="datos'.$numFil.'" value="'.$xrow.'"/></td>';
						
						if($this->botonModi!="")
							{
								$td = $td. '<td><a href="#" id="b'.$numFil.'" name="b'.$numFil.'" title="modificar"  value="..."  onclick="'.$this->modiClick.'"><img src="../../iconos/editamv.png" width="15" height="15" class="btnMod" /></a></td>';
							}
