...
function listar_servicios(pag){
	...
	new Ajax.Updater("lst_servicios", "../consultas/list_servicios.php?"+campos+"&pag="+pag,
	{  
	  onComplete: 
...
function nuevo_servicio(){
	...
	new Ajax.Updater("div_nservicio", "../consultas/n_servicio.php",
	{  
	  onComplete: 
	  function(){
	  	ubigeos();
...
function registrar_servicio(){
	...
	var flag = validar_servicio();

	if(flag==1){
	
		var campos =$("frm_nservicio").serialize(); 
		
		new Ajax.Request("../consultas/registrar_servicio.php?"+campos,
			{  
			  onComplete: 
			  function() { 
				 listar_servicios(1);
...
function modificar_servicio(id){
	...
	new Ajax.Updater("div_mservicio", "../consultas/m_servicio.php?id_servicio="+id,
	{  
	  onComplete: 
	  function(){
...
function mod_servicio(id){
	...
	if(flag==1){	
		...
		new Ajax.Request("../consultas/modificar_servicio.php?"+campos+"&id_servicio="+id,
			{  
			  onComplete: 
			  function() { 
...
function eliminar_servicio(id){
	...
	if (r == true) {
		...
		new Ajax.Request("../consultas/eliminar_servicio.php?id="+id,
			{  
			  onComplete: 
			  function() { 
				listar_servicios(pagina);
