...
$sqltt =mysql_query("SELECT * FROM tipokar",$conn) or die(mysql_error());
...
<script type="text/javascript">
	...
	function fcrearBloque()
	{
		...
		var idtipkar    = document.getElementById('idtipkar');
		...
		if(idtipkar.value=='' || _num_registros.value=='' ||_fec_ingreso.value=='')
		{alert('Falta ingresar datos');return;}
	    if(_num_kinicial.value>=(_num_registros.value)+1)
		{alert('El numero inicial no puede ser mayor al numero final');return;}
	...
	function fcreaBloqueKar()
	{
     /*
		...
		var idtipkar     = document.getElementById('idtipkar').value;
			...
			new Ajax.Request("../../kardex_generador.php?num_registros="+_num_registros+"&fec_ingreso="+_fec_ingreso+"&idtipkar="+idtipkar+"&num_kinicial="+_num_kinicial, 
			{
				method:'get',
		...
		*/	
		...
		var idtipkar     = document.getElementById('idtipkar').value;
		var kar = '';
		if(idtipkar==1){kar = 'ESCRITURAS';}
		if(idtipkar==2){kar = 'NO CONTENCIOSOS';}
		if(idtipkar==3){kar = 'TRANSFERENCIAS VEHICULARES';}
		if(idtipkar==4){kar = 'GARANTIAS MOBILIARIAS';}
		if(idtipkar==5){kar = 'TESTAMENTOS';}
		
		ajax=objetoAjax();
	
		ajax.open("POST", "../../kardex_generador.php",true);	
		ajax.onreadystatechange=function() {
			if (ajax.readyState==4 && ajax.status==200) {
				alert('Se guardo El Bloque del '+_num_kinicial+ ' al ' + _num_registros  + ' ' + kar + '.');
			}
		}
		ajax.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
		ajax.send("num_registros="+_num_registros+"&fec_ingreso="+_fec_ingreso+"&idtipkar="+idtipkar+"&num_kinicial="+_num_kinicial);
...
width:400; height:200;">
    ...
    <tr>
      <td height="30" bgcolor="#264965"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          ...
          <td width="328"><span class="titulosprincipales">Generar Bloque de Kardex</span></td>
          <td >&nbsp;</td>
            ...
            <tr>
              ...
              <tr>
                <td height="27" colspan="2" align="left">
                  <table  width="100%">
                    <tr>
                      <td width="30%" align="right"><span class="camposss">N.Kardex Inicial :</span></td>
                      <td width="70%">&nbsp;<input name="num_kinicial" type="text" id="num_kinicial" style="text-transform:uppercase"  value="1" size="15" /></td>
                    </tr>
                  </table>
                </td>
              </tr>
  ...
  <tr>
                <td height="27" colspan="2" align="left">
                  <table  width="100%">
                    <tr>
                      <td width="30%" align="right"><span class="camposss">Tipo de Kardex:</span></td>
                      <td width="70%"><select name="idtipkar" id="idtipkar" >
                      <?php
						
		  while($row=mysql_fetch_array($sqltt)){
