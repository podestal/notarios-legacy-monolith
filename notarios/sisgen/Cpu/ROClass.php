...
class RoClass {
	...
	public function loadData(){
		...
		$result = mysqli_query($sql);
		$this->_totalKardex2 = mysqli_affected_rows();
		//die(mysqli_affected_rows(). 'hola' );
		$affectedRowRo = 0;
		while($row = mysqli_fetch_assoc($result)){
			...
			foreach ($arrCodActos as  $item) {
				...
				$resultTipoActo = mysqli_query($sql);
				$rowTipoActo = mysqli_fetch_assoc($resultTipoActo);
	...
	public function  generateData(){
		...
		$resultRoDataField = mysqli_query($sql);
		$arrRoDataField = array();
		while($row = mysqli_fetch_assoc($resultRoDataField)){
		...
		$arrRo = array();
		$arrObj = array();
		$this->_totalKardex = mysqli_affected_rows();
		...
		while($row = mysqli_fetch_assoc($result)){
			...
			if($tipoEnvio == 'I'){
				...
				$resultDetalleBien = mysqli_query($sql);
				if(mysqli_affected_rows() != 0){
					$rowDetalleBien = mysqli_fetch_assoc($resultDetalleBien);
				...
				$resultMedioPago = mysqli_query($sql);
				$tipoActoMedioPago = 0;
				$arrObjItemRo = array();

				if(mysqli_affected_rows() != 0){

					while($rowMedioPago = mysqli_fetch_assoc($resultMedioPago)){
						$codeRow++;
						...
						$resultPatri = mysqli_query($sql2);	
						while($rowPatri = mysqli_fetch_assoc($resultPatri)){
				...
				$resultContratante = mysqli_query($sql);	
				$arrFileRow = array();	 
				...
				if(mysqli_affected_rows() != 0){
					while($rowContratante = mysqli_fetch_assoc($resultContratante)){
						$codeRow = $codeRow + 1;
						...
						if($rowContratante['uif'] == 'R' ){
							...
							$resultRepresentate = mysqli_query($sql);
							if(mysqli_affected_rows() != 0){
								$rowRepresentante = mysqli_fetch_assoc($resultRepresentate);
					...
					//	die($codTipoDocumento.'-'.$numDocumento);
						...
						$resultConyuge = mysqli_query($sql);
						if(mysqli_affected_rows() != 0 && $rowContratante['uif'] != 'R'){
							$rowConyuge = mysqli_fetch_assoc($resultConyuge);
