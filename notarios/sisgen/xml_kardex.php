...
function validarNombrePersonas($dato){
	$result =  preg_match('/\t/', $dato);
	$result +=  preg_match('/\</', $dato);
	$result +=  preg_match('/\>/', $dato);
	$result +=  preg_match('/\s\s/', $dato);
	$info = str_replace('/\s\s/',' ',$dato);
	$info=trim($dato," \t.");
...
function ValidarDocumentoNatural($tipo,$documento,$tipoInstrumento)
{
	$err = 0;
	$errormensaje = '';
...
function ValidarDocumentoJuridica($tipo,$documento,$tipoInstrumento)
{
	$err = 0;
	$errormensaje = '';
	if($tipoInstrumento == '2'){
    ...
    return array($err,$errormensaje,$tipo);
}
...
function PersonaNatural($x2,$isConyuge,$coduif){
	$fechanacimiento = '';//'2021-01-01';
	$arrProfesiones = array("114"=>"VISITADOR MEDICO"
		,"113"=>"VETERINARIO, ZOOLOGO, ZOOTECNICO"
		,"112"=>"VENDEDOR AMBULANTE"
		,"100"=>"SECRETARIA, RECEPCIONISTA, TELEFONISTA Y AFINES"
		,"099"=>"SACERDOTE / MONJA / RELIGIOSO"
		,"092"=>"PSICOLOGO/ TERAPEUTA"
		,"089"=>"PRODUCTOR DE CINE / RADIO / TELEVISION / TEATRO"
		,"085"=>"PILOTO"
		,"082"=>"PERIODISTA"
		,"999"=>"OTROS"
		,"998"=>"NO DECLARA"
		,"075"=>"MIEMBRO DE LA POLICIA / FUERZAS ARMADAS"
		,"073"=>"MEDICO / CIRUJANO"
		,"071"=>"MAYORISTA, COMERCIO AL POR MAYOR"
		,"070"=>"MARTILLERO / SUBASTADOR"
		,"068"=>"LIQUIDADOR, RECLAMACIONES/SEGUROS"
		,"066"=>"JUBILADO / PENSIONISTA"
		,"061"=>"INGENIERO"
		,"056"=>"ESTUDIANTE"
		,"078"=>"ENFERMERO / OBSTETRIZ / PARAMEDICO Y AFINES"
		,"052"=>"EMPRESARIO EXPORTADOR/ EMPRESARIO IMPORTADOR"
		,"115"=>"EMPRESARIO"
		,"051"=>"EMPLEADA (O) DEL HOGAR / NANA / GUARDIAN / PORTERO / PERSONAL DE LIMPIEZA Y AFINES"
		,"049"=>"ECONOMISTA"
		,"048"=>"DOCENTE"
		,"047"=>"DISTRIBUIDOR"
		,"046"=>"DEPORTISTA PROFESIONAL, ATLETA, ARBITRO, ENTRENADOR DEPORTIVO"
		,"045"=>"DENTISTA / ODONTOLOGO"
		,"044"=>"DECORADOR, DIBUJANTE, PUBLICISTA, DISEÑADOR DE PUBLICIDAD"
		,"042"=>"CORTE Y CONFECCION DE ROPA/ FABRICANTE DE PRENDAS"
		,"041"=>"CONTRATISTA"
		,"040"=>"CONTADOR"
		,"039"=>"CONSTRUCTOR"
		,"037"=>"CONDUCTOR/ CHOFER / TAXISTA/ TRANSPORTISTA Y AFINES"
		,"036"=>"COMERCIANTE / VENDEDOR"
		,"028"=>"CAMBISTA, COMPRA/VENTA DE MONEDA"
		,"024"=>"BACTERIOLOGO, FARMACOLOGO, BIOLOGO, CIENTIFICO Y AFINES"
		,"021"=>"ASISTENTE SOCIAL"
		,"019"=>"ARQUITECTO"
		,"015"=>"ANALISTAS / TECNICOS / PROGRAMADOR DE SISTEMA Y COMPUTACION"
		,"014"=>"AMA DE CASA"
		,"013"=>"ALBAÑIL / OBRERO / ELECTRICISTA / MECANICO / TECNICO  / OTROS OFICIOS"
		,"012"=>"AGRICULTOR / GANADERO Y AFINES"
		,"010"=>"AGENTE DE TURISMO/VIAJES"
		,"008"=>"AGENTE DE BOLSA"
		,"007"=>"AGENTE / INTERMEDIARIO / CORREDOR INMOBILIARIO"
		,"011"=>"AGENTE / INTERMEDIARIO / CORREDOR DE SEGUROS"
		,"006"=>"AEROMOZO/ AZAFATA Y AFINES"
		,"005"=>"ADUANERO/AGENTE DE ADUANAS/INSPECTORES DE FRONTERA"
		,"004"=>"ADMINISTRADOR"
		,"003"=>"ACTUARIO"
		,"002"=>"ACTOR / ARTISTA / ESCRITOR Y AFINES"
		,"001"=>"ABOGADO");
	$persona = "";
	...
	if($f=='N'){
		...
		$resultJuridica = validarNombrePersonas($xnombrecompleto);
		...
		$persona .="\t\t\t<DocsIdentificativos>\n";
			$persona .= "\t\t\t\t<DocIdentificativo>\n"; 
					$resulValida = ValidarDocumentoNatural($x2['tipodoc'],$x2['numdoc'],$tipoInstrumento);
					if($resulValida[0]==1){
					...
					if($x2['tipodoc']!=""){$persona .= "\t\t\t\t\t<TipoDocIdentidad>" .trim($doc). "</TipoDocIdentidad>\n";}  
		...
		if($x2['fechanaci']=="" and $isConyuge==0){
			if($fechanacimiento!=''){
				$persona .= "\t\t\t\t<FechaNacimiento>" .$fechanacimiento. "</FechaNacimiento>\n";
			}else{
		...
		if($x2['profesion']!="" and $x2['profesion']!=0){
			...
			$codprofesion = array_search(trim($x2['detaprofesion']), $arrProfesiones);
			if(($codprofesion==false || $codprofesion==0 || $codprofesion=="" ) && $isConyuge==0){
				array_push($arrKarObsPersona,$nombrecompleto.'->Falta profesion'.$x2['detaprofesion'].'--'.$codprofesion);
			}else{				
				if($codprofesion!=""){
					$persona .= "\t\t\t\t<Profesion>" .$codprofesion. "</Profesion>\n";
				}				
			}		
		}
...
function validarfolios($inicial,$final){
	$totalfolio = 1;
	$err = 1;
	if($inicial=='' && $final==''){
		$err = 0;
		...
		if(!is_numeric($inicial) || !is_numeric($final)){			
			$err = 0;
		}else if(is_float($inicial) || is_float($final)){
			$err = 1;
		}else{
			$err = 0;
			$totalfolio = intval($final) - intval($inicial);
		}
		if($totalfolio>99999){
			$err=1;
		}else if($totalfolio==0){
			$totalfolio=1;
		}else if($totalfolio<0){
			$totalfolio=1;
		}
...
function kardexml($conn,$tipoInstrumento,$Kardex,$idKardex){
	...
	while($x = mysqli_fetch_array($resultado)){ 
		...
		$resultfolio = validarfolios($x['folioini'],$x['foliofin']);
		if($resultfolio[0]==1){
			array_push($errorListKar, array('idkardex'=>$idkar,'kardex'=>$kar,'acto'=>'General','error'=>array('Error en el numero de folios') ));
		}
		...
		$salida_kardex_documento .= "\t\t<NumFolios>" . $resultfolio[1] . "</NumFolios>\n";	
		...
		if(mysqli_num_rows($resultado2) != 0 ) {
			...
			while($x2 = mysqli_fetch_array($resultado2)){ 
				array_push($arrPersonasEnKardex,$x2['id']);
				$resultPersona = PersonaNatural($x2,0,$x2['ROLUIF']);
				$resultConyugue = "";
				...
				if($x2['estc']=2 && ($x2['ROLUIF'] =="O" || $x2['ROLUIF'] =="B") && $x2conyuge!="" && $x2conyuge!="0"){
					$NombreConyuge[$x2conyuge]=$x2['nom'].' '.$x2['apepat'].' '.$x2['apemat'];
					array_push($BuscarConyuge,$x2conyuge);	
					$arrConyuge[$x2['idcontratante']] = $x2conyuge;		
				}
			} 
			if(count($BuscarConyuge)>0){
				foreach ($BuscarConyuge as $conyugueIntem) {
				    if(!in_array($conyugueIntem,$arrPersonasEnKardex)){
				    	$consultaConyugue = "SELECT  cl.idCliente, cl.idcliente AS id, cl.tipper AS tipp, cl.apepat AS apepat,
										...
										WHERE cl.idCliente = '$conyugueIntem'";
						...
						if(mysqli_num_rows($resultadoConyugue) != 0 ) {
							while($itemconyuge = mysqli_fetch_array($resultadoConyugue)){ 
								$resultConyugue = PersonaNatural($itemconyuge,1,'');
								$salida_kardex .= $resultConyugue[0];
								$arrKar = array_merge($arrKar,$resultConyugue[1]);
								$arrPersonasErr[$conyugueIntem] = $resultConyugue[2];											
							}
						}else{
							array_push($arrKar,$NombreConyuge[$conyugueIntem].'-> No se encontro al Conyuge, verifique el registro');
						}				
				    }
				}
			}
							...
							$salida_kardex .= "\t\t\t<PersonaJuridica id='".$x3['id']."'>\n";
									$salida_kardex .= "\t\t\t<DocsIdentificativos>\n";
										$salida_kardex .= "\t\t\t\t<DocIdentificativo>\n"; 
											$resulValida = ValidarDocumentoJuridica($x3['tipodoc'],$x3['numdoc'],$tipoInstrumento);
											$tipodocjuridica = $x3['tipodoc'];
											if($resulValida[0]==1){
												array_push($arrKar,$x3['razonsocial'].'->'.$resulValida[1]);
											} else{
												$tipodocjuridica = $resulValida[2];
											}
											if($tipodocjuridica!=""){
												$salida_kardex .= "\t\t\t\t\t<TipoDocIdentidad>" .$tipodocjuridica. "</TipoDocIdentidad>\n";
											}
											if($x3['numdoc']!=""){
									...
									if($x3['razonsocial']==""){										
										...
										$resultJuridica = validarNombrePersonas($x3['razonsocial']);
										if($resultJuridica[0]>0){
											array_push($arrKarObs,$x3['razonsocial'].'->Razon Social tiene caracteres no permitidos o espacios consecutivos');
										}
			...
			if(mysqli_num_rows($resultadokardex) != 0 ) {					
					...
					if($stop != 1){		
						...
						while($xxx = mysqli_fetch_array($resultadoD2All)){
							...
							if($xxx['repre']=='O' ){
								...
								if($xxx['fondos']!="" and $xxx['montoo']!="" and $resultIsUIFSUNAT == 1){
								...
								while($xxx1 = mysqli_fetch_array($resultadoD3)){
									...
									if($xxx1['inscrito'] == 1 && ($xxx1['idsedereg']!="" || $xxx1['numpartida']!="")){
								...
								while($xxx223 = mysqli_fetch_array($resultadoD3)){
									...
									if($xxx223['inscrito'] == 1 && ($xxx223['idsedereg']!=""||$xxx223['numpartida']!="")){
				...
				//********************Conyuge*********************************
							...
							if($conyugeNoInterviniente != null){
								$totalErrPersonasConyuge = count($arrPersonasErr[$conyugeNoInterviniente]);
								if($totalErrPersonasConyuge>0){
									array_push($errorListKar, array('idkardex'=>$idkar,'kardex'=>$kar,'acto'=>'Conyuge de '.$NombreConyuge[$conyugeNoInterviniente],'error'=>$arrPersonasErr[$conyugeNoInterviniente] ));
								}
								$salida_xml_sujeto_N .= "\t\t\t\t\t\t\t\t<NoInterviniente>\n";
						...
						if($salida_xml_sujeto_O == ''){
							array_push($arrKar,"Falta asignar el otorgante");
						}
						...
						while($xxx11 = mysqli_fetch_array($resultadoD11)){
							if($xxx11['repre']=='O' and $salida_xml_O == '' ){    
								...
								$salida_xml_O .= "\t\t\t\t\t\t\t<TipoIntervencion>1</TipoIntervencion>\n";
								...
								$salida_xml_B .= "\t\t\t\t\t\t\t<TipoIntervencion>2</TipoIntervencion>\n";
