...
$consulta_vehicular = "SELECT *, CAST(numescritura AS SIGNED) AS numescritura2 FROM kardex WHERE idtipkar='5' and fechaescritura <> '' 
					   AND STR_TO_DATE(fechaingreso,'%Y-%m-%d') >= STR_TO_DATE('".$desde."','%Y-%m-%d') 
					   AND STR_TO_DATE(fechaingreso,'%Y-%m-%d') <= STR_TO_DATE('".$hasta."','%Y-%m-%d')";
...
while($vehicular = mysql_fetch_array($ejecutar_vehicular)){
	...
	$arr_vehicular[$i][0] = $vehicular["fechaingreso"]; 
	...
	echo "<tr>
		...
		$ejecuta_clientes= mysql_query($consulta_clientes, $conn);
			while($rowcliente=mysql_fetch_array($ejecuta_clientes)){
			 echo strtoupper ($rowcliente['nombre'].$rowcliente['empresa'])."</br>";
			}
			echo"</span></td>
