<?php 
	...
	$consulpermiviaje = mysql_query("SELECT protesto.*, DATE_FORMAT(protesto.fec_notificacion,'%d/%m/%Y') AS 'fec_notificacion2' FROM protesto WHERE protesto.id_protesto='$id_poder'", $conn) or die(mysql_error());
...
//// quita el numero de cronologico
	function QuitaPod(){
		var _numformu 	  = document.getElementById('num_crono');
		var _numformu_ant = document.getElementById('num_formu_ant');
			...
			if(_numformu.value != '')
				...
				{
				 alert('No existe Nro.Acta a Eliminar');return; 	
				}
	}	
...
function fGeneraNumPoderQuitar()
{
	...
	$.post("quitarGeneraProtesto.php",{ id_poder : _id_poder}, function(_response){
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  ...
  <tr>
    ...
    <table  width="100%">
        ...
        <tr>
          <td colspan="5" align="center">.::GENERAR NRO. ACTA.</td>
          </tr>
        <tr>
          <td width="20%"><span class="camposss">Nro. Acta:</span></td>
          <td colspan="3"><div id="div_numcrono" style="width:100px;"><input name="num_crono" type="text" id="num_crono" style="text-transform:uppercase" size="15" readonly="readonly" placeholder="Autogenerado" value="<?php if($rowcpermiso['fec_crono']=='' or  $rowcpermiso['fec_crono']=='0000-00-00'){echo "";}else {echo $numkar2;} ?>" /></div></td>
        ...
        <tr>
          <td><span class="camposss">Fecha Notif :</span></td>
          <td colspan="3" valign="top"><label for="obs">
            <input name="fecha_crono" type="text" id="fecha_crono" style="text-transform:uppercase" size="15" value="<?php if($rowcpermiso['fec_crono']=='' or $rowcpermiso['fec_crono']=='0000-00-00'){echo date("d/m/Y");}else {echo $rowcpermiso['fec_crono']; }  ?>" />
          </label></td>
          <td valign="top">&nbsp;</td>
        </tr>
        <tr>
