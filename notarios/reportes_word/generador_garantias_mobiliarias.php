...
function garantias_mobiliarias(){
	...
	$rutaPlantilla="D:/plantillas/PROTOCOLARES/";
	$rutaDocumentoGenerado="C:/Proyectos/GarantiasMobiliarias/".$anioKardex."/";
	$txtListContratantes=array();
		
	//COMPROBACION DE SI EXISTE O NO EL DIRECTORIO DONDE SE CREARA EL DOCUMENTO
	if(!file_exists("C:/Proyectos/GarantiasMobiliarias/")){
		mkdir("C:/Proyectos/GarantiasMobiliarias/",0700);
	}
...
function get_data_contratantes($resultTransferencia){
	...
	foreach ($condiciones as $k => $v) {
		...
		if($v=='VENDEDOR' || $v=='PODERDANTE' || $v=="OTORGANTE" || $v=="REPRESENTANTE" || $v=="ANTICIPANTE" ||$v=="ADJUDICANTE" || $v=="DONANTE" || $v=="USUFRUCTUANTE" || $v=="TRANSFERENTE"){
		...
		if($v=='COMPRADOR' || $v=='APODERADO' || $v=="ANTICIPADO" || $v=="ADJUDICATARIO" || $v=="DONATARIO" || $v=="USUFRUCTUARIO" || $v=="TESTIGO A RUEGO" || $v=="ADQUIRIENTE" || $v=="DEUDOR"){
