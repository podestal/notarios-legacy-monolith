...
// Se verifica que formato de plantilla se usara.
	...
	$template = "plantilla_protocolar_disolucion.odt";
...
cliente2.numdoc ,(CASE WHEN(cliente2.detaprofesion<>"") THEN cliente2.detaprofesion ELSE ""  END) AS "Ocupacion", 
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	{
		...
		#busca conyuge:
		$consulconyuge1 = mysql_query('SELECT cliente.nombre AS "conyuge", cliente.numdoc,cliente.direccion  FROM cliente WHERE idcliente = (SELECT cliente2.conyuge FROM cliente2 WHERE idcontratante =  "'.$rowcontratantes[2].'") ', $conn) or die(mysql_error());
		...
		if($rowconyuge1 != "")
		{
			...
			$numdocconyuge 	  = $rowconyuge1[1];
			$direcconyuge	  = $rowconyuge1[2];
		}
		...
		{
			$interviniente = strtoupper(utf8_decode($rowcontratantes[3]));
		}else{
			$interviniente = "";
			}
		...
		{
			$participante = strtoupper(utf8_decode($rowcontratantes[3]));
		}
		else{$participante = "";}
		...
		if(($evalrepresentante == $evalua_rept) && ($evalua_rept != ""))
		{
			...
			$dataContratantes2[] = array('nom_contratante'=>" YO ".strtoupper(utf8_decode($rowcontratantes[3])).",IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".$rowcontratantes[6]." CON DOMICILIO EN ".utf8_decode($rowcontratantes[9]).", DISTRITO DE ".utf8_decode($rowcontratantes[10]).", PROVINCIA Y DEPARTAMENTO DE LIMA", 'nom_firma'=>strtoupper(utf8_decode($rowcontratantes[3])));
			
			$dataContratantes3[] = array('nom_contratante'=>" FIRMADO : ".strtoupper(utf8_decode($rowcontratantes[3]))." - DNI ".$rowcontratantes[6]." UNA FIRMA ILEGIBLE - UNA HUELLA DIGITAL.-"." ABOGADO QUE AUTORIZA : <DATOS ABOGADO QUE AUTORIZA>", 'nom_firma'=>strtoupper(utf8_decode($rowcontratantes[3])));
...
//Carga la plantilla;
	...
	$TBS->MergeBlock('d,e', $dataContratantes3);
