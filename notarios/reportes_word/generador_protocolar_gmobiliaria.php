...
// Se verifica que formato de plantilla se usara.
	$template = "plantilla_protocolar_gmobiliarias.odt";
...
WHERE contratantesxacto.kardex = "'.$num_kardex.'" AND cliente2.tipper="N"  AND contratantesxacto.idcondicion!="131" ORDER BY idcontratante ASC', $conn) or die(mysql_error());
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	{ 
		...
		$evalcondicion = "";
		if($rowcontratantes[12]=='470' )
			{
					$evalcondicion = "COMO CONSTITUYENTE "; 
			} // TRASNFERENTE
			
			else if($rowcontratantes[12]=='130')    
				{
					$evalcondicion = "COMO ACREEDOR "; // $evalcondicion = "COMO ADQUIRIENTE ";	
				} // ADQUIRIENTE  
			...
			if($rowjuridica[12]=='470')
				{
				$evalcondij= "COMO CONSTITUYENTE";
				}
			else if($rowjuridica[12]=='130')
				{$evalcondij= "COMO ACREEDOR";}
			else if($rowjuridica[12]=='128')
				{$evalcondij= "COMO DEUDOR";}
			else{$evalcondij= "";}
...
## EVALUA AL REPRESENTANTE:
		$consultrepre = mysql_query('SELECT contratantes.idcontratante FROM contratantes WHERE contratantes.kardex = "'.$num_kardex.'" AND contratantes.tiporepresentacion != "1"  AND contratantes.firma="1"', $conn) or die(mysql_error());
		//$rowrept =  mysql_fetch_array($consultrepre);	
		//$evalua_rept = $rowrept[0];
		
		
for($i = 0; $i <= $numcontratantes-1; $i++)//4
	{
		
		while($rowcontratantes =  mysql_fetch_array($consulcontratantes)){//ARROJA CON CONDICION Y TODOS LOS DATOS DE NATURALES
			
		$rowrept =  mysql_fetch_array($consultrepre);	// TODOS LAS PERSONAS MENOS AL APODERADO (REPRESENTANTE).272829
		
		//echo $rowcontratantes[2].'iguala'.$rowrept[0];
		...
		// NUEVO PARA EVALUACIÓN //30 //28 //27 (131,129,470)
		$evalcondicion = " ";
		...
		LEFT OUTER JOIN ubigeo ON ubigeo.coddis = cliente2.idubigeo
		WHERE contratantesxacto.kardex =  "'.$num_kardex.'" AND cliente2.tipper="N" ', $conn) or die(mysql_error());
		$rowcontratantes3 =  mysql_fetch_array($consulcontratantes3);	//230,228,227
		###################################
		###################################
...
for($i = 0; $i <= $numvehiculos-1; $i++)
	{
		...
		$dataVehiculos[] = array(
		"contenido_vehicular" => 
							 "PLACA: " .strtoupper($rowvehiculo[0]).", MARCA: " .strtoupper($rowvehiculo[1]).", CLASE: " .strtoupper($rowvehiculo[2]).strtoupper(utf8_decode(", AÑO DE FABRICACIÓN: ")).strtoupper($rowvehiculo[3]).strtoupper(utf8_decode(", SERIE N° ")).strtoupper($rowvehiculo[4]).", COLOR(ES): " .strtoupper($rowvehiculo[5]).strtoupper(utf8_decode(", MOTOR N° ")).strtoupper($rowvehiculo[6]).", MODELO: " .strtoupper($rowvehiculo[7]).", CARROCERIA: " .strtoupper($rowvehiculo[8]),
