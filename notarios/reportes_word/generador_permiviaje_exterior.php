...
$objTbs->VarRef['fecha'] = $codigonuevo;
...
$sql = "SELECT permi_viaje.id_viaje AS id_viaje,
				permi_viaje.num_kardex AS kardex, 
				permi_viaje.asunto  AS asunto,
				permi_viaje.fec_ingreso AS fecha_ingreso,
				permi_viaje.nom_recep AS nombreRecepcionista, 
				permi_viaje.hora_recep AS hora_recepcion,
				permi_viaje.referencia,
				permi_viaje.nom_comu  AS comunicarse,
				permi_viaje.email_comu  AS comunicarse_email,
				permi_viaje.documento,permi_viaje.num_crono AS numero_cronologico,
				permi_viaje.fecha_crono AS fecha_cronologico,
				permi_viaje.num_formu AS numero_formulario,
				permi_viaje.lugar_formu AS destino,
				permi_viaje.observacion, 
				permi_viaje.swt_est,
				permi_viaje.partida_e,
				permi_viaje.sede_regis,
				permi_viaje.via,
...
// print_r($FECHA_DESDE);return false;
...
{
	$notariolicencianueva='YO '.$row3['notario'].' ABOGADA - NOTARIA DE LIMA CON OFICIO NOTARIAL EN '.$row3['direccion_notario'];
	$objTbs->VarRef['licencia'] =$notariolicencianueva;
}

$fechitainterior = "SELECT permi_viaje.fec_ingreso AS fecha_ingreso FROM  permi_viaje WHERE permi_viaje.id_viaje = '$idPermiViaje'";
...
$objTbs->VarRef['nuevafechaletras'] = $fecha_letras_viaext;
...
while($row = mysql_fetch_assoc($result)){
	...
	$sql = "SELECT viaje_contratantes.c_condicontrat AS id_condicion,
		CONCAT(cliente.prinom, ' ', cliente.segnom, ' ', cliente.apepat, ' ', cliente.apemat) AS contratante,
		tipodocumento.destipdoc AS tipo_documento,
		tipodocumento.td_abrev AS abreviatura, 
		cliente.numdoc AS numero_documento, 
		IF(cliente.sexo='M',CONCAT(SUBSTR(nacionalidades.descripcion, 1,LENGTH(nacionalidades.descripcion)-1),'O'),CONCAT(SUBSTR(nacionalidades.descripcion, 1,LENGTH(nacionalidades.descripcion)-1),'A')) AS nacionalidad,  
		CONCAT(' CON DOMICILIO EN ',cliente.direccion) AS direccion,  
		viaje_contratantes.c_fircontrat,
		...
		(CASE 
		WHEN viaje_contratantes.condi_edad = 1 AND viaje_contratantes.edad != '' 
		...
		viaje_contratantes.partida_e AS partida,
		viaje_contratantes.sede_regis AS sede
	FROM viaje_contratantes 
	...
	if( $affectedRow != 0){
		while ($rowParticipante = mysql_fetch_assoc($resultParticipantes)) {
			...
			$rowParticipante['final'] = $rowParticipante['id_condicion'] == '003'?'DE ':'';
			
			
			if($rowParticipante['codigo_poderado']!= ''){
				$numero = $rowParticipante['codigo_poderado'];
				
				
				$sqlnuevo = "SELECT CONCAT(cliente.prinom, ' ', cliente.segnom, ' ', cliente.apepat, ' ', cliente.apemat) AS contratante,
						tipodocumento.td_abrev AS abreviatura,
						cliente.numdoc AS numero_documento
						FROM viaje_contratantes 
						INNER JOIN cliente ON cliente.numdoc = viaje_contratantes.codi_podera
						INNER JOIN tipodocumento ON tipodocumento.idtipdoc = cliente.idtipdoc
						WHERE viaje_contratantes.id_viaje = '$idPermiViaje' and codi_podera = '$numero'";
						
											
						$resultnuevo = mysql_query($sqlnuevo);
						$representados = mysql_fetch_assoc($resultnuevo);
						
				$rowParticipante['repre'] = 'EN REPRESENTACION DE '.$representados['contratante'] ;
				
			}
			else
			{
				$rowParticipante['repre'] = '';
			}
...
$sql = "SELECT viaje_contratantes.c_condicontrat AS id_condicion,CONCAT(cliente.prinom, ' ', cliente.segnom, ' ', cliente.apepat, ' ', cliente.apemat) AS contratante,
			 tipodocumento.destipdoc AS tipo_documento, tipodocumento.td_abrev AS abreviatura,cliente.numdoc AS numero_documento, nacionalidades.descripcion AS nacionalidad, 
...
$poder = mysql_fetch_array($sqlviaje);
	...
	//JSON
		 ...
		 $objPermisoViaje->codigoinstrumento = 251; 
		 $objPermisoViaje->codigonotaria ="123";
		 ...
		 END) AS campo,
		 SUBSTRING(N.desnacionalidad, 1, 3) segundo,C.`email`,C.`telcel`, C.`cumpclie`,
		 IF(V.`c_fircontrat` = "SI", TRUE , FALSE ) AS firma   FROM viaje_contratantes V 
		 INNER JOIN cliente C ON C.numdoc = V.c_codcontrat
		 INNER JOIN nacionalidades N ON N.idnacionalidad= C.nacionalidad WHERE id_viaje ="'.$idPermiViaje.'"', $conn) or die(mysql_error());
		...
		$objParticipante->condicion = $row3[7];
			...
			$objParticipante->fechanacimiento = $row3[11];
			...
			$objParticipante->firma = $row3[12];
			$objParticipante->nombrepresentante = '';
			$objParticipante->documentorepresentante = '';
			$objParticipante->partidaregistral = '';
			$objParticipante->sederepresentante ='';
			...
			'direccion'=>$objParticipante->direccion,
			'nacionalidad'=>$objParticipante->nacionalidad,'correo'=>$objParticipante->correo,
		...
		// define('WS_ENVIAR_DOCUMENTO','http://192.168.250.45/serviceqrdesarrollo/public/api/pviajes');
		// $ch = curl_init();  
		...
		//var_dump($objResponse['permisoviaje']);
...
$objTbs->VarRef['LOSELLA_PADRES'] = $losellaPadres;
...
$templateName = 'AUTORIZACION VIAJE MENOR EXTERIOR.docx';

// $templateName = 'PERMISO_VIAJE_EXTERIOR.odt';
...
if(empty($listParticipante['padre'])){
		
	$objTbs->MergeBlock('p',$listParticipante['apoderado']);

}else{
	
	if($listParticipante['padre'][0]['c_fircontrat'] == 'SI'){
		$objTbs->MergeBlock('p',$listParticipante['padre']);
	}
	else{
...
if(empty($listParticipante['madre'])){
	
	$objTbs->MergeBlock('r',$listParticipante['apoderado']);

}else{
	
	if($listParticipante['madre'][0]['c_fircontrat'] == 'SI')
	{
	$objTbs->MergeBlock('r',$listParticipante['madre']);
	}
	else{
	$objTbs->MergeBlock('r',$listParticipante['apoderado']);
	}
