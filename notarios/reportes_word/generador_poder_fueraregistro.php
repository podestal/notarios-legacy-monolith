...
//se crea el objeto  ClaseLetras
	...
	$fecha_letras_viaext = $fecha->fun_fech_letras($rowviaint[0]);
	// print_r($fecha_letras_viaext);return false;
...
// Se verifica que formato de plantilla se usara.
	$template = $rutaPlantilla."PODER FUERA DE REGISTRO BASE.docx";
...
$conteoj = mysql_num_rows($consulpoderdante);
	...
	$sqlfast=mysql_query('SELECT CONCAT(cliente.prinom," ",cliente.segnom," ",cliente.apepat," ",cliente.apemat)  AS "NOMBRE_APODERADO",cliente.numdoc as numero_documento
FROM poderes_contratantes 
INNER JOIN cliente ON cliente.numdoc = poderes_contratantes.c_codcontrat
...
$consulpoderdanten = mysql_query('SELECT  UPPER(CONCAT(cliente.prinom," ",cliente.segnom," ",cliente.apepat," ",cliente.apemat))AS "NOMBRE_APODERADO", 
...
#########################	
## TESTIGO A RUEGO
$consultestigo = mysql_query('SELECT UPPER(CONCAT(cliente.prinom," ",cliente.segnom," ",cliente.apepat," ",cliente.apemat))AS "NOMBRE_TESTIGO",
UPPER(tipodocumento.td_abrev) AS "TIP_DOC", 
UPPER(cliente.numdoc) AS "NUM_DOC", 
UPPER(tipoestacivil.desestcivil) AS "EST_CIVIL", 
UPPER(nacionalidades.descripcion) AS "NACIONALIDAD",
UPPER(cliente.direccion) AS "DIRECCION" , 
UPPER(CONCAT(atestiguados.prinom," ",atestiguados.segnom,", ",atestiguados.apepat," ",atestiguados.apemat))AS "ATESTIGUADO", d_tablas.des_item AS "INCAPACIDAD",
UPPER(documents_atesti.td_abrev) AS "TIP_DOC_ATES",  
UPPER(atestiguados.numdoc) AS "NUM_DOC_ATES", 
d_tablas.val_item AS "DES_INCAPACIDAD",
 poderes_contratantes.c_fircontrat AS "firma"
FROM poderes_contratantes 
INNER JOIN cliente ON cliente.numdoc = poderes_contratantes.c_codcontrat
INNER JOIN tipodocumento ON tipodocumento.idtipdoc = cliente.idtipdoc
...
else if($rownum_kar[0] != '')
{
	...
	$numcrono3 = substr($rownum_kar[0],5,6);
	$aniocrono = substr($rownum_kar[0],0,4);
}
// print_r($aniocrono);return false;	
...
while($rowfast = mysql_fetch_row($sqlfast))
	{
		...
		$dataCabecera[] = array(
			'poderdantesarriba' => str_replace("*","&",str_replace("?","'",strtoupper(utf8_decode($rowfast[0])))),
			'documento_poderdante' => $rowfast[1]
		);
...
{
	// $rowpoderdante3 = mysql_fetch_row($consulpoderdanten);
	$contadorPoderdante=1;
	
	while ($rowpoderdante2 = mysql_fetch_row($consulpoderdanten)){
		// print_r($rowpoderdante2);return false;
		...
		if($rowpoderdante2[9]=="F"){

			$nacionalidad = substr($rowpoderdante2[4],0,-1).'A';
		...
		}else if($rowpoderdante2[9]=="M"){

			$nacionalidad = substr($rowpoderdante2[4],0,-1).'O';
...
//Definicion de las variables del nuevo poderdante naural
	...
	$ocupacion	          	= strtoupper($rowpoderdante2[10]);
	$direccionopod          = strtoupper(utf8_decode($rowpoderdante2[5]));
    $ubigeopod				= strtoupper(utf8_decode($rowpoderdante2[8]));

	$nombreTestigo = strtoupper($rowpoderdante2[11]);
	$numeroDocumentoTestigo = strtoupper($rowpoderdante2[13]);
	$estadoCivilTestigo = strtoupper($rowpoderdante2[14]);
	$nacionalidadTestigo = strtoupper($rowpoderdante2[15]);
	$direccionTestigo = strtoupper($rowpoderdante2[16]);
	$ubigeoTestigo = strtoupper($rowpoderdante2[17]);
	$sexoTestigo = strtoupper($rowpoderdante2[18]);
	$ocupacionTestigo = strtoupper($rowpoderdante2[19]);
	...
	$identificado='';
		...
		if($sexo=="F"){
			...
			$datos_poderdante1 = 'Compareció por su propio nombre y derecho '.$nombre_poderdante.', '.strtoupper($nacionpod).', identificada con '.utf8_decode("DNI N° ").strtoupper(utf8_decode($numdocpod)).' <OCUPACION>, quien declara ser '.$estcivilpod.', con domicilio en '.$direccionopod.', del '.$ubigeopod;

			$identificado = 'identificada con DNI '.utf8_decode('N°');

		}else if($sexo=="M"){

			$datos_poderdante1 = 'Compareció por su propio nombre y derecho '.$nombre_poderdante.', '.strtoupper($nacionpod).', identificado con '.utf8_decode("DNI N° ").strtoupper(utf8_decode($numdocpod)).' <OCUPACION>, quien declara ser '.$estcivilpod.', con domicilio en '.$direccionopod.', del '.$ubigeopod;
	...
	$dataPoderdantes1['poderdantes'][] = array(
		'P_NOM'.$agregadoPoderdante=> $nombre_poderdante,
		'P_IDE'.$agregadoPoderdante=> $identificado,
		'P_DOC'.$agregadoPoderdante=> $numdocpod,
		'P_NACIONALIDAD'.$agregadoPoderdante=> $nacionalidad,
		'P_OCUPACION'.$agregadoPoderdante=> $ocupacion,
		'P_ESTADO_CIVIL'.$agregadoPoderdante=> 'quien declara ser '.$estcivilpod,
		'P_DOMICILIO'.$agregadoPoderdante=> 'con domicilio en '. $direccionopod.', del '.$ubigeopod,
		'P_SEXO'.$agregadoPoderdante=> $sexo,
...
for($i = 0; $i <= $numrepresentante; $i++)
	...
	$apoderadoPoder= utf8_decode($rowrepresenta[0].' IDENTIFICADA CON '.$rowrepresenta[1].' N° '.$rowrepresenta[2]);
	...
	$apoderadoPoder= utf8_decode($rowrepresenta[0].' IDENTIFICADO CON '.$rowrepresenta[1].' N° '.$rowrepresenta[2]);
	}
...
*/	
	...
	$nombre_marca     = "";
	$solicita         = utf8_decode($rowdatospoder[3]);
...
*/
		...
		if(count($objResponse['error'])>0){
			...
			// print_r($dataPoderdantes1);return false;
	$contratantesPoderdantes=array();
	foreach($dataPoderdantes1['poderdantes'] as $key => $value){ 
		$contratantesPoderdantes += $value;
	}
	$dataDocumentoWord[]  = $contratantesPoderdantes;
