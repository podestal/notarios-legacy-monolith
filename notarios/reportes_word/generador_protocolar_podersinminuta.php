...
// Se verifica que formato de plantilla se usara.
	...
	$template = "plantilla_protocolar_podersinminuta.odt";
...
tipoestacivil.desestcivil AS "ecivil", cliente2.direccion,  (CASE WHEN ISNULL(ubigeo.nomdis) THEN "" ELSE ubigeo.nomdis  END) AS "Distrito"  , cliente2.sexo AS "sexo" ,cliente2.tipper, contratantesxacto.idcondicion AS "condicion", IF(contratantes.firma = "0", "NO","SI") AS "firma",contratantesxacto.uif
...
## Objeto para juridicas.   -->>>>>>>>>>>>>>>>>>>>> ESTE VALEEEEEEEEEEEEEEEEEEEEE
#######################
...
cliente2.nombre, nacionalidades.descripcion AS "nacionalidad", tipodocumento.destipdoc AS "tipo_docu",
cliente2.numdoc , (CASE WHEN(cliente2.idprofesion<>"0") THEN profesiones.desprofesion ELSE ""  END) AS "Ocupacion", 
tipoestacivil.desestcivil AS "ecivil", cliente2.direccion,  (CASE WHEN ISNULL(ubigeo.nomdis) THEN "" ELSE ubigeo.nomdis  END) AS "Distrito"  , cliente2.sexo AS "sexo" ,cliente2.tipper, contratantesxacto.idcondicion AS "condicion", IF(contratantes.firma = "0", "NO","SI") AS "firma"
FROM contratantesxacto
INNER JOIN cliente2 ON cliente2.idcontratante = contratantesxacto.idcontratante
INNER JOIN contratantes ON contratantes.idcontratante = contratantesxacto.idcontratante AND contratantes.kardex = contratantesxacto.kardex
INNER JOIN nacionalidades ON nacionalidades.idnacionalidad = cliente2.nacionalidad
...
$contents2="NO SE HA DEFINIDO EL CUERPO";	
}
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	...
	// PEGAR PERSONA JURIDICA	
		...
		if($rowcontratantes[14]=='SI'){$eval_firma=strtoupper(utf8_decode($rowcontratantes[3]));}
		else{$eval_firma="";}
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	...
	$consulespecial = mysql_query('SELECT cliente2.nombre, nacionalidades.descripcion AS "nacionalidad", tipodocumento.destipdoc AS "tipo_docu", cliente2.numdoc,
UPPER(cliente2.detaprofesion) AS "Ocupacion", tipoestacivil.desestcivil AS "ecivil",
cliente2.direccion,  
IF(ubigeo.coddis="070101","DISTRITO DE CALLAO , PROVINCIA CONSTITUCIONAL DEL CALLAO",CONCAT("DISTRITO DE ",ubigeo.nomdis, ", PROVINCIA DE ", ubigeo.nomprov,", DEPARTAMENTO DE ",ubigeo.nomdpto )) AS "Distrito",contratantesxacto.uif
			FROM contratantes
			INNER JOIN cliente2 ON cliente2.idcontratante = contratantes.idcontratante
			...
			$rowespecial     = mysql_fetch_array($consulespecial);
		
		if($rowespecial[8]=="APO"){
						
				$nom_apoderado1=strtoupper(utf8_decode($rowespecial[0]));
		}else {
				$nom_apoderado1="";
			  }
