...
WHERE STR_TO_DATE(fechainicio,'%Y-%m-%d') >= STR_TO_DATE(NOW(),'%Y-%m-%d') AND STR_TO_DATE(fechafin,'%Y-%m-%d') <= STR_TO_DATE(NOW(),'%Y-%m-%d')";
...
{
	$notariolicencianueva='POR LICENCIA DE LA NOTARIA '.$row['notario'].' FIRMA EL NOTARIO '.$notariolicencia.' SEGUN RESOLUCION N° '.$notariolicencia2;
$objTbs->VarRef['licencia'] =$notariolicencianueva;
}
else
{
	$notariolicencianueva='YO '.$row['notario'].' ABOGADA - NOTARIA DE LIMA CON OFICIO NOTARIAL EN '.$row['direccion_notario'];
	$objTbs->VarRef['licencia'] =$notariolicencianueva;
}
...
if($row){
	foreach ($row as $key => $value) {
		...
		switch ($key) {
   				case 'fecha_ingreso':
   					# code...
   					$lettersFechaConclusion = $objNumeroLetra->fun_fech_completo2_anio($value);
		   			$bookmarkName = mb_strtoupper('LETRA'.'_'.$key);
...
while($row = mysql_fetch_assoc($result)){
	...
	$sql = "SELECT viaje_contratantes.c_condicontrat AS id_condicion,CONCAT(cliente.prinom, ' ', cliente.segnom, ' ', cliente.apepat, ' ', cliente.apemat) AS contratante,
			...
			IF(ubigeo.coddis='' OR ISNULL(ubigeo.coddis) ,'',CONCAT('DISTRITO DE ',ubigeo.nomdis, ', PROVINCIA Y DEPARTAMENTO DE ',ubigeo.nomdpto ))  AS ubigeo,
...
$poder = mysql_fetch_array($sqlviaje);
	...
	//JSON
		 ...
		 $consultapar = mysql_query('SELECT V.`c_fircontrat`,V.`c_codcontrat`, C.apepat,C.`apemat`, CONCAT (C.`prinom`," ",C.`segnom`) AS nombre , C.`idtipdoc`,C.`direccion`, 
		 (CASE
		...
		while($row3 = mysql_fetch_array($consultapar)){
			...
			'direccion'=>$objParticipante->direccion,'nacionalidad'=>$objParticipante->nacionalidad,'correo'=>$objParticipante->correo,
		...
		define('WS_ENVIAR_DOCUMENTO','http://200.60.145.200/serviceqr/public/api/pviajes');
		$ch = curl_init();  
		...
		if(count($objResponse['error'])>0){
			...
			$updatepermiviaje='UPDATE permi_viaje SET  qr = "1" WHERE id_viaje = "'.$idPermiViaje.'"';
...
$templateName = 'PERMISO_VIAJE_EXTERIORold.odt';

$fileName = 'PERMIVIAJE'.$arr_viaje[0];
$fileType = '.odt';
$here = getcwd();
...
$objTbs-> PlugIn (OPENTBS_CHANGE_PICTURE, 'descripcionImagen', $ruta);
	...
	$file_name3      = $path_exit.$fileName.$fileType;
