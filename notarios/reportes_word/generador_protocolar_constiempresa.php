...
// Se verifica que formato de plantilla se usara.
	...
	$template = "plantilla_protocolar_constiempresa.odt";
...
$consulfolio = mysql_query('select kardex.folioini, kardex.foliofin, kardex.papelini, kardex.papelfin, kardex.numminuta
from kardex where kardex.kardex = "'.$num_kardex.'" ', $conn) or die(mysql_error());
...
## PERSONA JURÍDICA A CREARSE : 
$consuljuridica = mysql_query('SELECT cliente2.razonsocial AS "new_empresa"
FROM cliente2
INNER JOIN contratantesxacto ON contratantesxacto.idcontratante = cliente2.idcontratante
WHERE contratantesxacto.kardex = "'.$num_kardex.'" AND cliente2.razonsocial <> "" AND cliente2.tipper="J" ', $conn) or die(mysql_error());
...
// # PERSONA JURÍDICA A SER CREADA

	$nom_newempresa   = strtoupper(utf8_decode(str_replace("/"," ",$rowjuridica[0])));
...
//Carga la plantilla;
	...
	$TBS->MergeBlock('b,c,h', $dataContratantes2);
