...
//se crea el objeto  ClaseLetras
	...
	$rutaPlantilla="D:/plantillas/EXTRAPROTOCOLARES/CERTIFICACIONES/ENTREGA DE CARTAS NOTARIALES/";
	$template = $rutaPlantilla."CERTIFICACION ENTREGA DE CARTA NOTARIAL.docx";
...
ingreso_cartas.conte_carta AS "contenido", 
DATE_FORMAT(STR_TO_DATE(ingreso_cartas.fec_entrega,"%d/%m/%Y"),"%Y/%m/%d") AS "fecha_diligencia",
...
//Definicion de las variables para llenar la plantilla dinamicamente
	...
	$fec_ingreso       = strtolower($fecha->fun_fech_letras($rowcartas[5]));
...
$poder = mysql_fetch_array($sqlpoder);
	...
	//JSON
		 ...
		 SUBSTRING(C.`docpaisemi`, 1, 3) segundo
		 ...
		 WHERE id_carta ="'.$num_carta.'"', $conn) or die(mysql_error());
			...
			$objParticipante->tipodocumento = '0'.$row3[1];
			$objParticipante->documento = $row3[2];
			$objParticipante->nombres = $row3[9];
			$objParticipante->apellidopaterno = $row3[7];
			$objParticipante->apellidomaterno = $row3[8];
			$objParticipante->direccion = $row3[3];
			$objParticipante->correo = '';
			$objParticipante->nacionalidad = $row3[10];
			$objParticipante->departamento =  $row3[6];
			$objParticipante->provincia =  $row3[5];
			$objParticipante->distrito =  $row3[4];
		...
		//define('WS_ENVIAR_DOCUMENTO','http://192.168.250.45/serviceqr/public/api/cartas');
		$ch = curl_init();  
...
//Nombre para el archivo a descargar.
    ...
    $file_name = $path_exit.$anioKardex[1].'/__CARTA__'.$num_carta.'.docx';
	$file_name_show = '__CARTA__'.$num_carta.'.docx';
