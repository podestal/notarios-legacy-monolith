...
if($row){
	foreach ($row as $key => $value) {
		...
		switch ($key) {
   				case 'fecha_ingreso':
   					# code...
   					$lettersFechaConclusion = $objNumeroLetra->fun_fech_comple2($value);
		   			$bookmarkName = mb_strtoupper('LETRA'.'_'.$key);
...
while($row = mysql_fetch_assoc($result)){
	...
	if( $affectedRow != 0){
		while ($rowParticipante = mysql_fetch_assoc($resultParticipantes)) {
			...
			$rowParticipante['domiciliado'] = $rowParticipante['sexo'] == 'M'?'DOMICILIADO':'DOMICILIADA';
...
$templateName = 'PERMISO_VIAJE_INTERIORold.odt';
