...
include_once('../includes/ClaseLetras.class.php');
 ...
 function holaacentos($rb){ 
        ...
        $rb = str_replace("ÃƒÃ¡", "á", $rb);
        $rb = str_replace("Ã±", "ñ", $rb);
        $rb = str_replace("Ã'", "Ñ", $rb);
        $rb = str_replace("ÃƒÃ±", "ñ", $rb);
        $rb = str_replace("Ã", "Ñ", $rb);
        $rb = str_replace("Ã??", "Ñ", $rb);
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	{ 
		...
		#busca conyuge:
		$consulconyuge1 = mysql_query('SELECT cliente.conyuge AS "conyuge", cliente2.detaprofesion as "ocupacion", cliente2.idestcivil AS "estado", cliente.direccion AS "direccion" FROM cliente
...
WHERE cliente2.idcontratante = "'.$rowcontratantes[2].'" ', $conn) or die(mysql_error());
		...
		//evalua conyuge:
		if($rowconyuge1['estado'] =="2")
		{
			$csql=mysql_query("SELECT nombre FROM cliente WHERE idcliente='".$rowconyuge1['conyuge']."'",$conn);
			$rs=mysql_fetch_array($csql);
			$nom_conyuge      = strtoupper($rs[0]);
			$formaconyuge     = "CON ".holaacentos(strtoupper($rs[0]));

		}
	
		else if($rowconyuge1['estado'] !="2")
		{
			$nom_conyuge      = "";
			$formaconyuge     = "";
		}
		...
		else if($rowconyuge1['ocupacion']!="")
		{
			$evalocupacion = $rowcontratantes[7];	
		}
		
		//evalua direcciones:
		if($rowconyuge1['direccion']=="")
		{
			$direccion = "NO ESPECIFICA";	
		}
		else if($rowconyuge1['direccion']!="")
		{
			$direccion = $rowconyuge1['direccion'];	
		}
		$evalcondicion = "";
...
WHERE cliente2.idcontratante = "'.$rowcontratantes[2].'"', $conn) or die(mysql_error());
		...
		if($rowcontratantes[12]!='188' && $rowcontratantes[12]!='189' && $rowcontratantes[12]!='117')
		{
				
		$dataContratantes2[] = array('nom_contratante'=>$evalsexo.$evalcondicion." ".str_replace("*","&",str_replace("?","'",holaacentos(strtoupper($rowcontratantes[3])))).", QUIEN MANIFIESTA SER DE NACIONALIDAD ".$rowcontratantes[4].", DE ESTADO CIVIL ".$rowcontratantes[8]." ".holaacentos($formaconyuge).", DE OCUPACION Y/O PROFESION ".($evalocupacion).", DOMICILIAR EN ".($direccion).", ".($rowcontratantes[10]).", SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".$rowcontratantes[6]." Y DECLARA QUE PROCEDE POR DERECHO PROPIO", 'nom_firma'=>str_replace("*","&",str_replace("?","'",strtoupper(($rowcontratantes[3])))));
			## -> .strtoupper($rowrepderecho[2])." ".strtoupper($rowrepderecho[4])
			
				if($rowcontratantes['condicion']=='006' || $rowcontratantes['condicion']=='043' || $rowcontratantes['condicion']=='049' || $rowcontratantes['condicion']=='055' || $rowcontratantes['condicion']=='113' || $rowcontratantes['condicion']=='188') # TRANSFERENTE
					{
						$eval_x_persona_transferente = holaacentos(strtoupper($rowcontratantes[3]));
						$dataContratantes3[] = array('nom_transferente'=>$eval_x_persona_transferente);
							...
							{
								$eval_x_persona_adquiriente = holaacentos(strtoupper($rowcontratantes[3]));
...
IF(ubigeo.coddis="070101","DISTRITO DE CALLAO , PROVINCIA CONSTITUCIONAL DEL CALLAO",CONCAT("DISTRITO DE ",ubigeo.nomdis, ", PROVINCIA DE ", ubigeo.nomprov,", DEPARTAMENTO DE ",ubigeo.nomdpto )) AS "Distrito",contratantes.numpartida as numpartida
			...
			{
					$dataContratantes2[] = array('nom_contratante'=>$evalsexo.$evalcondicion." ".str_replace("*","&",str_replace("?","'",holaacentos(strtoupper($consulrepresentantenatural1[0])))).", QUIEN MANIFIESTA SER DE NACIONALIDAD ".$consulrepresentantenatural1[1].", DE ESTADO CIVIL ".$consulrepresentantenatural1[5]." ".holaacentos($formaconyuge).", DE OCUPACION Y/O PROFESION ".($consulrepresentantenatural1[4]).", DOMICILIAR EN ".($consulrepresentantenatural1[6]).", ".($consulrepresentantenatural1[7]).", SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".$consulrepresentantenatural1[3]." Y DECLARA QUE PROCEDE EN REPRESENTACION DE ".$rowcontratantes[3]." SEGUN FACULTADES INSCRITAS EN LA PARTIDA ELECTRONICA Nº".$consulrepresentantenatural1['numpartida']." DEL REGISTRO DE MANDATOS Y PODERES DE LIMA", 'nom_firma'=>str_replace("*","&",str_replace("?","'",strtoupper(($rowcontratantes[3])))));
					...
					## -> .strtoupper($rowrepderecho[2])." ".strtoupper($rowrepderecho[4])
						...
						if($rowcontratantes[12]=='006' || $rowcontratantes[12]=='043' || $rowcontratantes[12]=='049' || $rowcontratantes[12]=='055' || $rowcontratantes[12]=='113' || $rowcontratantes[12]=='188') # TRANSFERENTE
							{
								$eval_x_persona_transferente = strtoupper($rowcontratantes[3]);
