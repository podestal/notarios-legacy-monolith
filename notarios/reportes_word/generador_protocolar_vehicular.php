...
//$contents2 = holaacentos(docx2text($archivo_ruta));
...
for($i=0;$i<=$num_insertos-1;$i++)
	{
		...
		$contents2_inserto = strtoupper(holaacentos(docx2text($ruta_inserto.trim($values[$i]).".docx")));
...
IF(ubigeo.coddis="070101","DISTRITO DE CALLAO , PROVINCIA CONSTITUCIONAL DEL CALLAO",CONCAT("DISTRITO DE ",ubigeo.nomdis, ", PROVINCIA DE ", ubigeo.nomprov,", DEPARTAMENTO DE ",ubigeo.nomdpto )) AS "Distrito"
		...
		if($rowjuridica[12]=='114')
		{
			...
			$evalcondij= "COMO COMPRADOR:";
			}else {
			//	$evalcondij= "COMO TRANSFERENTE";
				$evalcondij= "COMO VENDEDOR:";
				}
		if($rowjuridica[10]=='J')
		{
		...
		$dataRepresentantes[] = array('nom_contratante' => $evalcondij." ".holaacentos($evalseguim)." ".str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($rowrepresentante[0])))).", SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".strtoupper(holaacentos($rowrepresentante[3])).", QUIEN MANIFIESTA SER DE NACIONALIDAD ".strtoupper($rowrepresentante[1]).", DE ESTADO CIVIL ".strtoupper($rowrepresentante[5]).", DE OCUPACION Y/O PROFESION ".strtoupper(holaacentos($rowrepresentante[4])).", DOMICILIAR EN ".strtoupper(holaacentos($rowrepresentante[6])).", ".strtoupper(holaacentos($rowrepresentante[7])).strtoupper(holaacentos(", Y DECLARA QUE PROCEDE EN REPRESENTACI�N DE ")).str_replace("*","&",str_replace("?","'",strtoupper($rowjuridica[3])))." CON REGISTRO UNICO DE CONTRIBUYENTE ".strtoupper($rowjuridica[6]).strtoupper(holaacentos(" CON FACULTADES INSCRITAS EN LA PARTIDA ELECTR�NICA NUMERO ")).strtoupper($rowjuridica[11])."  DEL REGISTRO DE PERSONAS JURIDICAS DE LIMA", 'nom_firma'=>str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($rowrepresentante[0])))).chr(13).chr(10)." POR ".str_replace("*","&",str_replace("?","'",strtoupper($rowjuridica[3]))));
		...
		{	
		$eval_mostrar_personax = str_replace("*","&",str_replace("?","'",strtoupper($rowjuridica[3]))) . " REPRESENTADA POR " . strtoupper(holaacentos($rowrepresentante[0]));
		...
		{	
		$eval_mostrar_personay = str_replace("*","&",str_replace("?","'",strtoupper($rowjuridica[3]))) . " REPRESENTADA POR " . strtoupper(holaacentos($rowrepresentante[0]));
		#$eval_mostrar_personax = "";
		}
		// PARA MOSTRAR ARRIBA :
		//$dataRepresentantes_muestra[] = array('mues_nom_contratante' => strtoupper($rowjuridica[3]) . " REPRESENTADA POR " . strtoupper(holaacentos($rowrepresentante[0])));
...
for($i = 0; $i <= $numcontratantes-1; $i++)
	{
		...
		$dataContratantes2[] = array('nom_contratante'=>$evalsexo.$evalcondicion." ".str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($rowcontratantes[3])))).", QUIEN MANIFIESTA SER DE NACIONALIDAD ".$rowcontratantes[4].", DE ESTADO CIVIL ".$rowcontratantes[8]." ".holaacentos($formaconyuge).", DE OCUPACION Y/O PROFESION ".strtoupper(holaacentos($evalocupacion)).", DOMICILIAR EN ".holaacentos($rowcontratantes[9]).", ".holaacentos($rowcontratantes[10]).", SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".$rowcontratantes[6]." , QUIEN PROCEDE POR SU PROPIO DERECHO", 'nom_firma'=>str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($rowcontratantes[3])))));
			## -> .strtoupper($rowrepderecho[2])." ".strtoupper($rowrepderecho[4])
			$consultapersona2=$rowcontratantes[4];
				if($rowcontratantes[12]=='006' || $rowcontratantes[12]=='043' || $rowcontratantes[12]=='049' || $rowcontratantes[12]=='055' || $rowcontratantes[12]=='113' || $rowcontratantes[12]=='188') # TRANSFERENTE
					{
						$eval_x_persona_transferente = strtoupper(holaacentos($rowcontratantes[3]));
						$dataContratantes3[] = array('nom_transferente'=>$eval_x_persona_transferente);
							...
							{
								$eval_x_persona_adquiriente = strtoupper(holaacentos($rowcontratantes[3]));
...
IF(ubigeo.coddis="070101","DISTRITO DE CALLAO , PROVINCIA CONSTITUCIONAL DEL CALLAO",CONCAT("DISTRITO DE ",ubigeo.nomdis, ", PROVINCIA DE ", ubigeo.nomprov,", DEPARTAMENTO DE ",ubigeo.nomdpto )) AS "Distrito"
			...
			{
					$dataContratantes2[] = array('nom_contratante'=>$evalsexo.$evalcondicion." ".str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($consulrepresentantenatural1[0])))).", SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".$consulrepresentantenatural1[3].", QUIEN MANIFIESTA SER DE NACIONALIDAD ".$consulrepresentantenatural1[1].", DE ESTADO CIVIL ".$consulrepresentantenatural1[5]." ".holaacentos($formaconyuge).", DE OCUPACION Y/O PROFESION ".holaacentos($consulrepresentantenatural1[4]).", DOMICILIAR EN ".holaacentos($consulrepresentantenatural1[6]).", ".holaacentos($consulrepresentantenatural1[7])." Y DECLARA QUE PROCEDE EN REPRESENTACION DE ".$rowcontratantes[3], 'nom_firma'=>str_replace("*","&",str_replace("?","'",strtoupper(holaacentos($rowcontratantes[3])))));
...
for($i = 0; $i <= $numvehiculos-1; $i++)
	{
		...
		$dataVehiculos[] = array(
		'contenido_vehicular' => 
							 "PLACA: " .strtoupper($rowvehiculo[0]).", MARCA: " .strtoupper($rowvehiculo[1]).", CLASE: " .strtoupper($rowvehiculo[2]).strtoupper(holaacentos(", A�O DE FABRICACI�N: ")).strtoupper($rowvehiculo[3]).strtoupper(holaacentos(", SERIE N� ")).strtoupper($rowvehiculo[4]).", COLOR(ES): " .strtoupper($rowvehiculo[5]).strtoupper(holaacentos(", MOTOR N� ")).strtoupper($rowvehiculo[6]).", MODELO: " .strtoupper($rowvehiculo[7]).", CARROCERIA: " .strtoupper($rowvehiculo[8])	
								);	
	}
...
$contratante = str_replace("*","&",str_replace("?","'",holaacentos($nom_contratante))).", QUIEN MANIFIESTA SER DE NACIONALIDAD ".holaacentos($nacionalidad).", DE ESTADO CIVIL ".$est_civil." ".holaacentos($formaconyuge).", DE OCUPACION Y/O PROFESION ".$ocupacion.", DOMICILIAR EN ".holaacentos($domicilio).", DISTRITO DE ".holaacentos($nom_distrito).", PROVINCIA Y DEPARTAMENTO DE LIMA, SE IDENTIFICA CON DOCUMENTO NACIONAL DE IDENTIDAD NUMERO ".holaacentos($numdoc_contrat). " , QUIEN PROCEDE POR SU PROPIO DERECHOOO";
$consultapersona1=holaacentos($nacionalidad);

//AGREGAMOS SI ES NACIONALIDAD PERUANA

 
 if( strtoupper($consultapersona1)=="PERUANA"){
	 $pos=true;
 }else{
	 if(strtoupper($consultapersona2)=="PERUANA"){
		 $pos=true;
	 }
...
if($pos==true){
$consultapersona="DOY FE QUE EL COMPARECIENTE CUMPLE CON LOS REQUISITOS ESTABLECIDOS EN EL INCISO ¨H¨ DEL ARTICULO 54 DE LA LEY DEL NOTARIADO, QUE HE DADO CUMPLIMIENTO AL ARTICULO 27 Y QUE NO QUE NO CONOCIENDO A LOS COMPARECIENTES HE CUMPLIDO CON IDENTIFICARLOS CON LOS DOCUMENTOS POR ELLOS PRESENTADOS Y HABER SEGUIDO EL PROCEDIMIENTO DEL ARTÍCULO 55 LITERAL A)  DE LA MISMA LEY,";
//$consultapersona="CONOCIENDO AL COMPARECIENTE HE CUMPLIDO CON IDENTIFICARLO CON LOS DOCUMENTOS POR EL PRESENTADOS Y HABER SEGUIDO EL PROCEDIMIENTO DEL ART. 55 A) y C) DE LA MISMA LEY (REQUERIMIENTO DE OTROS DOCUMENTOS QUE PERMITAN SU IDENTIDAD)  MIENTRAS SE IMPLEMENTE EL REGISTRO DE CARNÉ DE EXTRANJERÍA, PASAPORTES Y CARNÉ MIGRATORIO DE INGRESO DE EXTRANJEROS)";	

}else{
$consultapersona="CONOCIENDO AL COMPARECIENTE HE CUMPLIDO CON IDENTIFICARLO CON LOS DOCUMENTOS POR EL PRESENTADOS Y HABER SEGUIDO EL PROCEDIMIENTO DEL ART. 55 A) y C) DE LA MISMA LEY (REQUERIMIENTO DE OTROS DOCUMENTOS QUE PERMITAN SU IDENTIDAD)  MIENTRAS SE IMPLEMENTE EL REGISTRO DE CARNÉ DE EXTRANJERÍA, PASAPORTES Y CARNÉ MIGRATORIO DE INGRESO DE EXTRANJEROS)";	
}

 $consultapersona=utf8_decode($consultapersona);
