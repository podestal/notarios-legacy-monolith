...
// Se verifica que formato de plantilla se usara.
	...
	$rutaPlantilla="D:/plantillas/EXTRAPROTOCOLARES/CERTIFICACIONES/CERTIFICACION DE APERTURA DE LIBROS/";
	

	// $template = $path_template."plantilla_libro".$extension;
	// $template = $rutaPlantilla."CERTIFICACION APERTURA DE LIBRO BASE.docx";
	$orientacionH = $_REQUEST["orientacionH"];
	$orientacionV = $_REQUEST["orientacionV"];


	if($orientacionH=='true'){
		$template = $rutaPlantilla."CERTIFICACION APERTURA DE LIBRO HORIZONTAL.docx";
	}
	if($orientacionV=='true'){
		$template = $rutaPlantilla."CERTIFICACION APERTURA DE LIBRO VERTICAL.docx";
	}
	// print_r($template);
	// return false;
	//echo $template; exit;
...
$consullibros = mysql_query('SELECT libros.numlibro, 
				libros.descritiplib, 
				libros.empresa,  
				libros.ruc, 
				libros.folio, 
				tipofolio.destipfol, 
				libros.numlibro AS "num_crono", 
				CONCAT(libros.apepat," ",libros.apemat,", ",libros.prinom," ", libros.segnom) AS "nombre",
				(CASE WHEN (libros.tipper="N") THEN libros.ruc ELSE libros.ruc END) AS "documento",
				(CASE WHEN (libros.tipper="N") THEN "D.N.I." ELSE "R.U.C." END) AS "tipo_documento" ,
				nlibro.idnlibro, 
				nlibro.desnlibro , 
				libros.solicitante, 
				libros.dni, 
				...
				LEFT JOIN cliente ON cliente.idcliente = libros.codclie
				LEFT JOIN ubigeo ON ubigeo.coddis = cliente.idubigeo
				LEFT JOIN usuarios on usuarios.idusuario=libros.idusuario
				WHERE libros.numlibro =   "'.$num_libro.'" and libros.ano = "'.$anioLibro.'"', $conn) or die(mysql_error());
...
//Definicion de las variables para llenar la plantilla dinamicamente
	...
	$num_crono        		= $rowlibro[6];
	$ano_crono				= $rowlibro[23];

	// $usuario				=$rowlibro[24];
	// $dni_usuario			=$rowlibro[25];
	$detalle			    =$rowlibro[26];
	...
	$num_doc       = $rowlibro[8];
	// print_r($documento);return false;
	$num_doc_plantilla		= ($rowlibro[27]=='')?'':$rowlibro[27];
	...
	if(empty($num_doc)){
		...
		$num_doc 			= $num_doc;
		$domicilio_fiscal 	= 'CON DOMICILIO EN '.utf8_decode($rowlibro[22]);
		$ubigeo 			= utf8_decode('DEL DISTRITO DE '.$rowlibro[19].' PROVINCIA DE '.$rowlibro[20].' Y DEPARTAMENTO DE '.$rowlibro[21]);
	}
	
	
	$tipo_documento         = $rowlibro[9];
	$nro_libro		        = $rowlibro[10];
	// $des_nro_libro		    = $rowlibro[11];
	if($rowlibro[11]=='PRIMERO'){
		$des_nro_libro 		='APERTURA';
	}else{
		$des_nro_libro		= $rowlibro[11];
	}
	...
	$dni_solici		        = utf8_decode('IDENTIFICADO CON DNI N°'.$rowlibro[13]);
	//$nom_dist         = $rowdistrito[1];
	
	
	
	if($des_libro=='CUADERNO DE OBRA'){
		$titulo_libro = 'Apertura de Cuaderno de Obra';
		$tipo_libro = 'CUADERNO';
		$solicitud_libro = '';
		$solicitud_obra = 'SOLICITADO POR';
		$obra = 'OBRA: "'.$detalle.'"';
		$solicitante_libro = '';
		$solicitante_obra =  $nombre_solici.', '.$dni_solici;
		$calidad_obra =  'EN SU CALIDAD DE RESIDENTE';
		// $template = $rutaPlantilla."CERTIFICACION APERTURA CUADERNO DE OBRA BASE.docx";
	}else{
		$titulo_libro = 'Apertura de Libro';
		$tipo_libro = 'LIBRO';
		$solicitud_libro = 'a solicitud de ';
		$solicitud_obra = '';
		$obra = '';
		$solicitante_libro = $nombre_solici.', '.$dni_solici;
		$solicitante_obra = '';
		$calidad_obra = '';
	}
	...
	// print_r($rowlibro[17]);return false;
	$fecha_letras_viaext2 = $fecha->fun_fech_letras($rowlibro[17]);
	
	$fec_letras_completa= strtoupper(utf8_decode($fecha_letras_viaext2));
	$newDate = date("d/m/Y", strtotime(utf8_decode($rowlibro[17])));
	$fec_completa= strtoupper($newDate);
	...
	if($rowlibro[16]=="J")
		{
				...
				if(empty($num_doc)){
					...
					$tipo_persona = 'CON RUC NUMERO';
				}
		}
		else{
				$tipo_persona = 'CON DNI NUMERO';
		}
	##########################################################
...
FROM libros L
LEFT JOIN notarios N ON N.idnotario = L.idnotario  WHERE L.numlibro ='".$num_libro."' and L.ano = '".$anioLibro."'", $conn) or die(mysql_error());
...
$poder = mysql_fetch_array($sqlpoder);
	...
	//JSON
		 ...
		 $objPermisoViaje->folioinicial = 'x';
		 $objPermisoViaje->foliofinal ='x';
		 $objPermisoViaje->direccion ='x';
		 ...
		 dni, solicitante,comentario2 FROM libros  WHERE numlibro like '".$num_libro."' and libros.ano = '".$anioLibro."'", $conn) or die(mysql_error());
			...
			$objParticipante->departamento =  'x';
			$objParticipante->provincia =  'x';
			$objParticipante->distrito =  'x';
			$objParticipante->representante_cartapoder = 'x';
			$objParticipante->descripcionacartapoder = 'x';
			$objParticipante->cargo = 'x';
			$objParticipante->partidaregistral = 'x';
			$objParticipante->sederegistral = 'x';
			$objParticipante->asiento = 'x';
			$objParticipante->observacion = 'x';
		...
		//define('WS_ENVIAR_DOCUMENTO','http://192.168.250.45/serviceqr/public/api/libros');
		$ch = curl_init();  
...
//Nombre para el archivo a descargar.
	...
	if(!file_exists($path_exit.$anioLibro)){
		mkdir($path_exit.$anioLibro,0700);
	}
	
    $file_name = $path_exit.$anioLibro.'/__LIBRO__'.$num_libro.'-'.$anioLibro.'.docx';
	$file_name_show = '__LIBRO__'.$num_libro.'-'.$anioLibro.'.docx';
